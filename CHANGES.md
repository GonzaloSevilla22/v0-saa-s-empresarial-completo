...
- **`billing-edge-effective-plan`** ✅ (2026-07-31, sign-off PO mismo día) — cierra la brecha entre `get_effective_plan` (arriba) y las Edge Functions: `_shared/ai-quota.ts`, `ai-precio/index.ts` y `generate-export/index.ts` reimplementaban una copia vieja de la regla de trial leyendo `public.profiles` (columna `profiles.billing_plan` nunca actualizada desde su default `'gratis'` — C-19 en adelante la autoridad de billing vive en `accounts`), por lo que una cuenta que pagó o exenta era tratada como plan gratis por las 8 Edge Functions de IA + exportaciones. **Governance CRÍTICO** (34 cuentas reales, una con pago reconciliado) — sign-off explícito del PO sobre 4 Open Questions antes del apply.
  ...
  - **Leer antes**: `openspec/changes/billing-edge-effective-plan/design.md` (D1-D7), `openspec/changes/archive/.../billing-pro-trial/` (o su design.md si aún no archivado) para `get_effective_plan`.
