...
Historial de compras de un cliente — una fila por operación de venta con su total canónico, agregación resuelta en el backend y paginación estándar — más la superficie de detalle del cliente (`/clientes/[id]`) que lo expone junto a la cuenta corriente ya existente (C-30), a la que le da por fin puerta de entrada desde la lista de clientes.
...
El resumen SHALL calcularse sobre la totalidad de las operaciones del cliente y NO SHALL depender de la página del historial que se esté visualizando. El resumen SHALL reutilizar la misma definición de agregados que alimenta el listado de clientes con actividad (`client-activity`), sin declarar una segunda fórmula.
