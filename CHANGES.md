...
- OQ-A — `bank_movements` para `transfer`/`card` en el POS: requiere UX de "¿a qué cuenta bancaria entró?"; candidato natural es un change junto a `bank-reconciliation` (ya completa).
- OQ-B — asiento contable discriminado por forma de pago en `journal-entry-outbox`: el outbox ya recibe `payment_method` en el payload de `SaleConfirmed`, el consumidor puede evolucionar sin tocar el hot path.
- OQ-C — opt-in de caja en el form de venta común (checkbox, solo con sesión abierta y fecha=hoy).
- OQ-D — form de venta con `kind='credit'` posteando el cargo (primero revisar datos reales de OQ-2 en producción).
- OQ-E — mismo tratamiento para compras/pagos a proveedor (`supplier_accounts`) — mecánico, los helpers `c30_*_supplier_*` ya existen.
