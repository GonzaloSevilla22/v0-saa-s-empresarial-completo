...
- [x] 8.8 **Agregada post-activación (2026-08-02), follow-up chico** — la cola de conciliación (`GET /payments/subscriptions/ambiguous` + `POST .../resolve`, PR #345) quedó implementada admin-only por REST sin pantalla; el PO preguntó dónde estaba. Se agregó:
  - `/admin/pagos/ambiguas` (`frontend/app/(dashboard)/admin/pagos/ambiguas/page.tsx`): lista la cola (fecha, plan, monto, motivo de ambigüedad, `preapproval_id`), selector de cuenta destino (`AccountSearchCombobox`, búsqueda server-side) y botón "Asignar" → `resolve` → notice de éxito o error inline por fila → la fila se refresca (`invalidateQueries`) tras un resolve OK. Estado vacío ("No hay suscripciones pendientes de revisión") y degradación limpia para no-admin (gating client-side vía `profiles.role`, mismo patrón que `/admin/pagos`). Entrada nueva en el sidebar admin.
  - Endpoint nuevo `GET /payments/accounts/search?q=` (admin-only, `min_length=2`) + `BillingRepository.search_accounts` — no existía ningún endpoint de búsqueda de cuentas; se agregó porque el selector de destino lo necesitaba (`public.accounts` no tiene nombre propio, se busca por email/nombre del owner vía `auth.users`/`profiles`). Sin migración — solo lectura sobre tablas existentes.
  - **Aviso por campana: NO implementado, documentado como pendiente deliberado.** El sistema de notificaciones (`_notification_from_event`/`_notification_audience`, `v3-notifications-realtime`) es fundamentalmente **por cuenta**: `notifications.account_id` es `NOT NULL REFERENCES accounts(id)`, la RLS de lectura filtra por `account_id IN (current_account_ids())`, y el target `ADMIN` resuelve a los **owners de esa cuenta** — no a `profiles.role='admin'` de plataforma. Una fila `ambiguous` nace **sin** `account_id` por definición (D2bis) — no hay ninguna cuenta a la que dirigir el aviso con el mecanismo actual. Notificar al admin de plataforma requeriría (a) una migración de schema (account_id nullable o pseudo-cuenta "plataforma") + (b) tocar `_notification_audience`/`_notification_from_event`, compartidas por 8 tipos de evento ya vivos en producción, + (c) revisar la RLS de `notifications` para que un admin sin membresía en ninguna cuenta relevante pueda leerlo. Es governance CRÍTICO (núcleo de notificaciones en producción) y excede el alcance de un follow-up de UI — la pantalla es la prioridad de esta task. Queda para un change dedicado si el PO lo pide.
