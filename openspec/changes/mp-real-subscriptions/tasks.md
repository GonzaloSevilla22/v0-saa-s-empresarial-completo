...
- [ ] 5.7 **CRITERIO DE ACEPTACIÓN, no opcional — pendiente post-merge**: dado que **todas las 34 cuentas tienen `plan_expires_at=NULL` hoy** (5.1) y D6 garantiza que NULL no degrada, la comparación antes/después debería dar **cero diferencias** — pero la verificación real (recapturar y comparar fila por fila) se hace después de que este PR mergee y CI aplique la migración a producción, no antes. Queda como paso obligatorio de la verificación prod read-only post-merge de este PR.
