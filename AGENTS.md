...
**No hay ningún change activo pendiente de apply o archive** — `qa-integral-modulos` (ítem 17) quedó archivado el 2026-09-02, junto con `seguros-perfil-asesor`, `caja-compras-cobranzas` (ítem 19), `cobranzas-reverso` (ítem 20) y `cobranzas-catalogo-pagos` (ítem 21), los cinco ya cerrados; con la saga de cobranzas la lista queda **completa**. El gating de fondo sigue siendo `v3-rbac-multirole` (ítem 4, CRÍTICO, bloqueado a sign-off del PO; su dependencia `v31-tenancy-pool-rls` ya está **satisfecha**). Lo que quedó documentado y sin resolver:
