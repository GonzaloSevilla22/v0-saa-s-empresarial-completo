...
22. **`cobranzas-panel`** 🔨 **APPLY COMPLETO 2026-09-02** (propose PR **#494** · apply en rama `opsx/cobranzas-panel-apply`, PR abierto el mismo día — **pendiente: merge + verificación post-merge + humo real del PO + archive**). **Etapa A del módulo de cobranzas**: tablero `/cobranzas` de deudores (RPC nueva `rpc_receivables_report`, migración `20261021000001`, molde de `rpc_payment_method_report` con guard `P0401`), `GET /reports/receivables` + `/summary` (D2: el resumen agrega sobre el MISMO RPC — un solo predicado de deudor), KPI "Por cobrar" en la grilla diaria del Tablero (D6, `KpiCard` gana prop `href`), columna Saldo + acceso a la cuenta en `/clientes` (D9: `LEFT JOIN` 1:0..1 en `_classified_activity_cte`, hot path intacto — SAFETY NET 53→59 backend / 25→30 frontend sin regresiones). Cero escritura de dinero; Cobrar reutiliza `RegisterPaymentForm` en Dialog y la invalidación de `receivables.*` vive en los hooks de mutación (D8, 9 mutaciones). D4 normativo: "Último cargo"/"Último cobro", nunca mora; reversa no rejuvenece; días ART. OQs firmadas: OQ-1/OQ-2 → Etapa B (`cobranzas-vencimientos`, ya propuesta en PR #496); OQ-3 KPI sin BranchFilter; OQ-4 adjustment aparece con "—". Verificación: backend 1766/1766 (cov 91,43%), frontend 1855/1857 (2 = AdminSegurosPage flaky-preexistente), 40 gates SQL (39+nuevo PASS, único FAIL el artefacto local conocido), visual real en 4 combinaciones con stack local completo. Hallazgo lateral: `test_cobranzas_reverso.sql` nunca se cableó a CI (candidato); el gate nuevo sí quedó en `KPI_Validation.yml`. Detalle completo en `CHANGES.md`.
...
**Quedan cuatro changes activos**: dos gateados a tareas `[MANUAL PO]` y no a código pendiente — `v31-authz-token-hook` (44/48 tasks — falta activar el hook de auth en el Dashboard de Supabase, tasks 8.3-8.6) y `v31-mp-upgrade-webhook-fix` (35/38 tasks — falta decidir OQ1 la ventana de convivencia y retirar el webhook legacy, tasks 6.1-6.3) — más **`cobranzas-panel` (ítem 22, APPLY COMPLETO 2026-09-02** en rama `opsx/cobranzas-panel-apply`; pendiente: merge del PR + verificación post-merge + humo real del PO + archive — grupos 8-9 de sus tasks) y **`cobranzas-vencimientos`** (Etapa B, propuesta en PR #496, 0 tasks aplicadas — **no aplicar hasta cerrar la Etapa A**). Todo lo demás propuesto está archivado: `qa-integral-modulos` (ítem 17), `seguros-perfil-asesor`, `caja-compras-cobranzas` (ítem 19), `cobranzas-reverso` (ítem 20), `cobranzas-catalogo-pagos` (ítem 21) y `mp-real-subscriptions` (suscripciones recurrentes de MercadoPago, ✅ archivada 2026-09-02, ver `CHANGES.md`) ya cerrados. El gating de fondo sigue siendo `v3-rbac-multirole` (ítem 4, CRÍTICO, bloqueado a sign-off del PO; su dependencia `v31-tenancy-pool-rls` ya está **satisfecha**). Lo que quedó documentado y sin resolver:
...
Ninguno de los quince es urgente por sí solo; quedan para que el PO decida si ameritan un change propio (posiblemente combinados, como se hizo con h1+h2 en `tenancy-guard-caja-outbox`).
