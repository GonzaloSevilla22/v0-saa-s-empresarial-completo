...
Pool de conexiones asyncpg con JWT-passthrough para el backend FastAPI. Gestiona el ciclo de vida del pool (startup/shutdown), inyecta los claims JWT del usuario como parámetros de sesión PostgreSQL (`SET LOCAL app.jwt_claims`) en cada conexión adquirida, y expone la dependencia `get_db_conn` para que los endpoints reciban conexiones ya configuradas con RLS org-based activa.
