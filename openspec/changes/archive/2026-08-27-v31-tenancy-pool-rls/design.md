...
### Amendment 2026-08-26/27 — Cierre del rollout (26) y archivado del change (27): K5 cerrado

> **Ambas palancas ON en prod.** Paso 1 (`TENANCY_TX_SCOPE_ENABLED`) encendido y confirmado por el PO el 2026-08-23 (corría de facto desde antes); Paso 2 (`TENANCY_RLS_ROLE_ENABLED`) encendido por el PO el 2026-08-24 ~19:14 UTC (PR #453). La ventana de observación de la condición 3 (`design.md` §"Probado bajo carga") se **comprimió de 7 días a 48 h por decisión firmada del PO el 2026-08-23** — el incidente de la fuga multi-tenant #446 hacía más urgente activar la RLS efectiva que completar el plazo original, y los filtros explícitos de #446 ya daban defensa en profundidad durante la espera. Los cuatro contadores de la condición 4 cerraron en cero durante esa ventana; el smoke E2E del corte del Paso 2 (grupo 8, `tasks.md`) fue limpio salvo tres bugs de formulario/UX ajenos a tenancy (#463/#464/#465). Detalle completo por tarea en `tasks.md` grupos 4/5/8/9.

**Cierre de K5** (el 500 intermitente de compras que motivó este change, ver Context arriba): **RESUELTO**. Cero ocurrencias observadas con el Paso 1 activo, tanto durante las semanas de operación de facto previas a la confirmación formal del PO (4.4) como durante la ventana de 48 h del corte del Paso 2 (5.4/8.5) — incluidas 15 compras creadas correctamente inmediatamente después del corte. La hipótesis mecánica de Context (claims con alcance de sesión aterrizando en una conexión física distinta bajo Supavisor en transaction mode) queda confirmada por la desaparición del síntoma tras el fix del alcance (D1), no sólo por ausencia de observación. Actualizado también en `audit/codigo-backend.md` (K5 pasa de `CONFIRMADO (abierto)` a `RESUELTO`).

**Paso 3 y deuda de limpieza**: quedan registrados como candidatos futuros en `CHANGES.md` (ficha `v31-tenancy-pool-rls`), no en este change — ver esa ficha para el criterio de arranque del rol de login dedicado (OQ-2 opción (b)) y para la deuda de retirar las dos palancas y el camino de código viejo.
