...
- [x] 13.1 `CustomerAccountHistory.tsx`: acción "Anular" por fila (`isReversible`), bloqueada con motivo (`isReversalBlocked`), `MOVEMENT_LABELS`/`MOVEMENT_ICONS` con `payment_received_reversal`. **Hallazgo real**: la pantalla usa `get_account()` → `repo.list_movements()` (limit/offset), NO `list_movements_page()` — los 4 derivados (`is_reversible`/`is_reversal_blocked`/`has_cash_movement`/`has_bank_movement`) tuvieron que sumarse TAMBIÉN ahí o "Anular" nunca habría aparecido en la pantalla real (corregido, commit separado). `clientId` pasa a ser prop requerida; wireado en `/clientes/[id]/cuenta/page.tsx`.
- [x] 13.2 `SupplierAccountHistory.tsx`: espejo, con `payment_made_reversal`. Mismo hallazgo aplicado a `list_movements` de proveedor. `supplierId` requerido; wireado en `/proveedores/[id]/cuenta/page.tsx`.
- [x] 13.3 Diálogo de confirmación vía `getDeleteCompensation({...}, party, "cobro"|"pago")`: "se repondrá la deuda..." (ítem propio, no reutiliza `hasAccountCharge`) + "se revertirá el asiento contable" (siempre, D5) + caja/banco sólo si aplican (derivados `has_cash_movement`/`has_bank_movement`, mismo molde que `purchase_repository.py` — `payment_method` NO sirve, D3). Campo de motivo opcional vía `reasonField` nuevo en `DeleteOperationDialog` (compartido, extendido con `actionVerb`/`actionVerbGerund`/`icon`/`reasonField` opcionales — defaults preservan los 3 consumidores existentes).
- [x] 13.4 Verificado: `Math.abs(m.amount)` se preserva sin cambios; los tests `customer-account-history-sign.test.tsx`/`supplier-account-history-sign.test.tsx` (extendidos con el mock de la mutación, no reescritos) siguen en verde — el bug del PO 2026-08-21 no se reintrodujo.
- [x] 13.5 `/caja`: verificado por lectura de código (`caja/page.tsx` consume `CASH_MOVEMENT_META`/`CASH_MOVEMENT_FAMILIES` genéricamente, sin lista hardcodeada propia) — los dos tipos nuevos aparecen automáticamente por construcción, sin tocar `caja/page.tsx`.
- [x] 13.6 Verificación **estructural** (no capturas reales — el humo real con navegador queda para el PO, mismo trato que el resto de los changes recientes cuando el entorno bloquea el login real): las clases responsive (`sm:hidden`/`hidden sm:grid`) siguen el patrón preexistente e idéntico de los dos componentes; los tonos nuevos (`destructive`/`success` para las dos reversas, incluida la corrección de un ícono que usaba `text-emerald-400` literal en vez del token `text-success`) pasan el gate `token-contrast-aa.test.ts` (35/35) sin cambios — WCAG AA garantizado por construcción, ya que no se introdujo ningún color literal nuevo. Claro/oscuro: los dos temas comparten los mismos tokens semánticos (`--success`/`--destructive` resueltos por CSS var), sin ninguna rama de código condicionada al tema.
- [x] 13.7 Accesibilidad: la acción habilitada gana `aria-label={actionVerb + " " + label}` (nueva, en `DeleteOperationDialog` — mejora también a los 3 consumidores existentes); el estado bloqueado expone el motivo como `aria-label` (no sólo `title`, verificado con `toHaveAccessibleName`, RED genuino confirmado); el campo de motivo tiene `<Label htmlFor>` asociado a un `id` real. Foco atrapado: heredado de Radix `AlertDialog`, sin cambios.
- [x] 13.8 `pnpm tsc --noEmit`: cero errores en los archivos tocados (verificado por grep dirigido; los ~50 errores preexistentes del árbol — `e2e/*.spec.ts` sin `@playwright/test` resuelto, y 6 tests ajenos con mismatches de tipo previos — son independientes de este change). Frontend: **1809/1811 passed** (1 archivo, `AdminSegurosPage.test.tsx`, 2 tests) — confirmado **flaky preexistente bajo carga** (9/9 en corrida aislada), documentado en CLAUDE.md.
