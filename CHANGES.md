...
- **`billing-pro-trial`** ✅ (2026-07-31, PR #313) — `public.get_effective_plan(account_id)` como definición NORMATIVA única del plan efectivo (precedencia exención → trial vigente → `billing_plan` → `gratis` fail-closed); trial PRO de 30 días para cuentas nuevas; exención de cortesía auditable en `accounts`; aviso de excedente de plan vía outbox.
