...
El inventario completo es una tarea del paso 2, y cada colisión se resuelve por una de dos vías, decidida caso por caso: **(a)** encaminar la escritura por un RPC `SECURITY DEFINER` — coherente con DEC-24 y preferible cuando la escritura tiene invariantes de negocio; **(b)** agregar la policy de escritura faltante — preferible para tablas de infraestructura donde el predicado de tenencia es trivial. Lo que **no** es aceptable es descubrir la colisión el día del corte.
...
3. **7 días naturales corridos de operación real** con el paso 1 activo, que incluyan al menos **dos cierres de caja** y un día de actividad alta. Se registra el volumen real de requests del período **como número observado**, no contra un umbral inventado.
4. **Contadores en cero durante esos 7 días**: cero ocurrencias del 500 intermitente de compras (K5), cero `idle_in_transaction_session_timeout`, cero 403 anómalos de "cuenta no encontrada" (el síntoma de claims ausentes), cero errores de transacción abortada en los logs de Render.
...
5. Inventario de escrituras directas contra `pg_policies` (D7) y resolución de cada colisión (RPC o policy nueva). Si aparecen policies faltantes, van en una migración idempotente propia.
6. Encender la palanca del paso 2 en un horario de baja actividad, con el PO presente.
...
- **OQ-1 — ¿7 días es el período correcto de observación del paso 1?** La condición 3 propone 7 días naturales con al menos dos cierres de caja. Es una elección de criterio, no un resultado medido: el proyecto no tiene una línea de base de volumen documentada. Opciones: **(a)** 7 días como se propone; **(b)** menos días pero exigiendo un umbral de requests servidos (habría que medir primero cuál es el volumen semanal típico); **(c)** más días si el PO prefiere atravesar un cierre de mes. *El sign-off del 2026-07-30 dice "probado bajo carga" sin fijar la ventana.*
- **OQ-2 — ¿Rol de login dedicado como paso 3?** El cambio de rol por transacción (D6) es la opción simple y reversible, pero **falla abierto** si un camino de código omitiera el cambio de rol. Un rol de login dedicado sin BYPASSRLS falla **cerrado**, a cambio de replicar GRANTs, separar los pools y verificar el formato de usuario que acepta Supavisor en el plan contratado. Opciones: **(a)** quedarse con el cambio por transacción; **(b)** planificar un paso 3 con rol dedicado una vez que el paso 2 esté estable; **(c)** ir directo al rol dedicado en el paso 2, asumiendo la incertidumbre de infraestructura en el corte. *Recomendación: (b) — obtener el beneficio ahora y la garantía después, sin apostar los dos a la vez.*
- **OQ-3 — Resolución de las colisiones de D7: ¿RPC o policy?** Cuatro colisiones ya identificadas (`fiscal_documents`, `cashboxes`, `events`, `email_logs`) y un inventario completo pendiente. Encaminar por RPC es más coherente con DEC-24 pero más caro; agregar la policy es más barato pero suma superficie de escritura directa alcanzable también desde PostgREST (el navegador). *No se decide en el propose: cada caso se resuelve en el paso 2 con su justificación, y los que impliquen exponer escritura directa a `authenticated` sobre tablas sensibles (`fiscal_documents`) se elevan al PO antes de aplicarse.*
- **OQ-4 — ¿Se comunica la ventana del paso 2 a los usuarios?** El corte es transparente si el inventario está completo, y el rollback es inmediato. Pero es el cambio con más superficie del cluster. ¿Se elige una ventana horaria anunciada, o se hace en baja actividad sin aviso?
