...
- [ ] 7.5 Gates de CI verdes: `validate-kpis`, vitest, pytest (≥87%), Playwright E2E, Vercel — **pendiente de confirmar en el PR real vía `gh pr checks --watch`**.

## 8. Cierre

- [ ] 8.1 Verificar en prod, tras el merge, que la migración se aplicó una sola vez (`MAX(version)`) y que las 4 RPCs existen sin overload y con sus GRANTs.
- [ ] 8.2 Smoke manual en prod: alta de una venta y de una compra con forma de pago, edición preservando el método, y reporte cuadrando contra el listado. Ejecutar `scripts/sql/backfill_payment_method_pos_sales.sql` con conteos antes/después.
- [ ] 8.3 Actualizar `CHANGES.md` y dejar registradas en el design las respuestas del PO a las OQ; abrir el change siguiente recomendado (`pos-catalogo-pagos`).
