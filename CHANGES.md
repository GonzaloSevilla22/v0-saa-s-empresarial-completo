...
- **Candidatos que deja**: migrar las dos copias JS de "top productos" (`supabase/functions/ai-insights`, `frontend/lib/ai/buildBusinessSnapshot.ts`) a `rpc_product_ranking`; migrar los 3 reportes existentes a `components/charts/`; **E3** (export CSV, `rpc_product_sales_evolution` + `/estadisticas/productos/[id]`, `ai-estadisticas` — grupos 8-10 de `tasks.md`); filtro de canal sin superficie (`p_canal` existe en RPCs y API, ninguna pantalla lo expone); al archivar, reconciliar la spec `product-ranking` ("ranking por categoría") con la forma real (5ª dimensión de `rpc_sales_breakdown`); 12.6 (verificar en prod `MAX(version) = 20261024000001`, ACLs y el conteo 218 → 0 de `last_sale_date` corridos) y 14.3 (demo real del PO) tras el merge.
- **Leer antes**: `openspec/changes/estadisticas-ventas/design.md` (D1-D15, OQs firmadas y notas de implementación de E1 y de E2), las cabeceras de `supabase/migrations/20261024000001_estadisticas_ventas_e1.sql` y `20261025000001_estadisticas_ventas_e2.sql` (checkpoints vivos + desvíos declarados).
