# authz-token-claims Specification

## Purpose
Contrato de los claims de autorización que el hook de Supabase Auth (`custom_access_token_hook`) inyecta en el JWT de acceso: qué claims existen (rol de plataforma, rol de tenant, plan efectivo), de qué tabla sale cada uno, cómo se resuelve la cuenta activa cuando hay multi-membresía, qué pasa ante un error de cálculo (degradar sin romper el login, dejando rastro), en qué superficie se habilita/deshabilita en producción, y cómo se verifica que los claims llegan sin exponer el token. Implementado en `v31-authz-token-hook` (H-07 de la auditoría 2026-07-07).
