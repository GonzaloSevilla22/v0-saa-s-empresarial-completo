...
- [x] 8.1 Confirmado `MAX(version)` = `20260925000001` (timestamp de la segunda migración) contra prod real, vía MCP SELECT — no contra el estado del workflow (que además pasó en verde: "Build and Deploy" #32148960439, job "Deploy Supabase" completó `Deploy Database Migrations` sin error).
- [x] 8.2 Confirmado 35/35 cuentas con flag efectivo activo (`COALESCE(f.enabled,true)`), y 35 filas con `enabled=true` explícito en `account_feature_flags` (el UPSERT corrió sobre las 35 cuentas reales de prod, no sólo las 26 que ya tenían fila).
- [x] 8.3 Confirmado 0 `operation_created` huérfanos remanentes (re-derivado con la misma consulta COALESCE/NOT EXISTS de la migración) y 0 líneas con `account_id` nulo en `sale_items`/`purchase_items`. Confirmado además, vía `pg_get_functiondef`, que la `rpc_create_purchase_operation` desplegada en prod contiene `v_flag_on` y el `INSERT INTO public.purchase_items` condicionado — el hallazgo de 2.2 quedó reflejado en la función real, no sólo en el archivo de migración.
- [ ] 8.4 **T0 + 24 h** — PENDIENTE: requiere que transcurran 24 h reales desde el merge (2026-08-18) para tener población de ventas/compras nuevas que verificar. No ejecutable dentro de esta sesión. Queda como seguimiento explícito para el PO/equipo: repetir la consulta de D2 (ventas/compras creadas después de la migración, con producto y sin línea — esperado 0) el 2026-08-19 o después.
- [ ] 8.5 Condicional a 8.4 — no aplica todavía (nada que rollbackear sin el resultado de la verificación estagiada).
- [x] 8.6 Cierre de OQ-4 (`clientes-frecuentes-historial`, orden por defecto) y OQ-6 (`admin-kpi-refresh`, código muerto) registrado en este `tasks.md` y en el PR #413. **Pendiente actualizar `CHANGES.md`** con la entrada de este change (fuera del alcance de una sesión de apply — se deja para el paso de archive o un commit de documentación aparte). Deudas abiertas con dueño: OQ-1 (ruta de edición sin líneas, recomendado change propio con prioridad) y OQ-3 (RPCs `get_admin_*` sin consumidor) — ambas heredadas de `design.md`, mas la nueva OQ-5 (sign-off del hallazgo de compras, ya en el PR).
