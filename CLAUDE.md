...
16. **`gastos-forma-pago`** 🔨 **PROPUESTA 2026-08-28 — apply pendiente** (artefactos en `openspec/changes/gastos-forma-pago/`; **governance MEDIUM con un tramo de severidad ALTA**: escribe dinero en caja y en banco). Pedido textual del PO: *"Modificar Módulo Gastos para incluir identificación del gasto, ejemplo efectivo, transferencia y que estos movimientos concilien caja y banco"* — coincide con la deuda ya anotada como OQ-4 de `pos-banco-movimientos`. El gasto es **el único documento operativo que no dice con qué se pagó**: 175 gastos en prod ($8.723.710,63) sin un solo movimiento de caja ni bancario, y `cash_movements` con `movement_type='expense'` = **0** aunque el `CHECK` lo acepta desde C-28 y la UI de /caja ya sabe mostrarlo. **Estructural, no una columna**: hoy es `INSERT` plano + `DELETE` crudo sin RPC ni guards, y por DEC-24 dos libros atómicos exigen RPC `SECURITY DEFINER` → alta + edición + borrado en una sola etapa (etaparlo dejaría el sistema peor: es el agujero de `delete-guard-ledgers`). **Cero helpers nuevos**: la pata de caja delega en `c28_register_cash_movement` (el guard de tenencia de `tenancy-guard-caja-outbox` cubre a este caller **sin una línea nueva**) y la bancaria en `_pay_register_operation_bank_movement`, copiando verbatim los predicados de venta y compra. **Bloqueante destapado**: 0 de 37 catálogos tienen `bank_account_id` y sólo 4 de 37 cuentas tienen banco activo → sin D5 el pedido del PO fallaría en silencio para el 100% de los tenants. Decisiones de peso: opt-in de caja **pre-marcado** con las tres condiciones del form de venta (D1), `kind='credit'` rechazado (D3, `expenses` no tiene contraparte), **inmutabilidad `P0423`** en vez de contra-movimiento (D11), `expense_reversal` nuevo, con el patrón de contra-movimiento con tipo propio de `sale_reversal` pero **signo opuesto** y misma familia de filtro (D9), `P0426` al borrar en efectivo con la caja cerrada (D8). **Cero ERRCODEs nuevos.** Históricos **sin imputar y sin backfill** (D7). Fuera de alcance declarado: asiento contable (V2.6), eventos al outbox, soft delete, forma de pago en el importador. Superficie completa sobre `/gastos` (ruta y menú ya existen) + cierra dos bugs preexistentes (el alta descarta `branch_id`, incumpliendo RN-93 en 175/175; la edición borra `cost_center_id` en silencio). Migración prevista `20261015000001`. **1 capability nueva (`expense-operation`) + 5 modificadas · 15 grupos / 117 tasks TDD-first · 19 decisiones · 11 OQs con recomendación.** Una revisión adversarial (2026-08-29, 17 hallazgos) corrigió los artefactos antes del apply: el guard de signo de la compensación de caja se **invierte** (copiado verbatim de la venta es falso para todo gasto y nunca lanzaría `P0426`), la fecha se compara como `date` y no con un cast dependiente del TimeZone de la sesión, `rpc_payment_method_report` necesita `DROP FUNCTION` + re-`GRANT` (42P13 + ACLs reseteadas), el listado de `/gastos` **migra a `GET /expenses` paginado** porque el lock es un derivado que PostgREST no puede traer (D18), `P0412` se mapea a 422 con `field` sólo en el camino de gasto (D19), y el módulo Gastos **sí tiene tests hoy** (safety net dirigido, no andamio). Detalle completo en `CHANGES.md`.
...
**Hay un change propuesto y sin aplicar: `gastos-forma-pago` (ítem 16)** — es lo siguiente en la fila. Ni `sucursal-guard-vaciado-auditoria` (ítem 12) ni los dos changes de tenancy (ítems 13 y 14) dejaron un change propio recomendado como "siguiente", y el gating de fondo sigue siendo `v3-rbac-multirole` (ítem 4, CRÍTICO, bloqueado a sign-off del PO; su dependencia `v31-tenancy-pool-rls` ya está **satisfecha**). Lo que además quedó documentado y sin resolver:
...
Ninguno de los diez es urgente por sí solo; quedan para que el PO decida si ameritan un change propio (posiblemente combinados, como se hizo con h1+h2 en `tenancy-guard-caja-outbox`).
