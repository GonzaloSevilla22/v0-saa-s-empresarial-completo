-- 20260311000010_fix_rpc_admin_stats_community.sql
-- Fix scope of community_stats CTE in rpc_admin_module_stats
...
BEGIN
  ...
  CASE p_module_type
    ...
    WHEN 'comunidad' THEN
      -- Create a single statement or redefine CTE per query
      SELECT jsonb_build_object(
        'users_count', COUNT(DISTINCT user_id),
        'count', COUNT(*),
        'avg_per_user', CASE WHEN COUNT(DISTINCT user_id) > 0 THEN (COUNT(*)::numeric / COUNT(DISTINCT user_id)::numeric)::numeric(10,1) ELSE 0 END
