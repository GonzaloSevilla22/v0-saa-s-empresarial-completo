...
-- Drop old vulnerable signatures
DO $$
BEGIN
...
-- get_dashboard_financials (no p_user_id — uses auth.uid())
...
BEGIN
  ...
  RETURN QUERY
  WITH ingresos AS (
    SELECT COALESCE(SUM(amount), 0) AS total FROM public.sales
    WHERE user_id = v_uid AND date >= p_date_from AND date <= p_date_to
  ),
  gastos AS (
  ...
  compras AS (
    SELECT COALESCE(SUM(amount), 0) AS total FROM public.purchases
    WHERE user_id = v_uid AND date >= p_date_from AND date <= p_date_to
  )
  SELECT i.total, g.total, c.total, (i.total - (g.total + c.total))
  FROM ingresos i CROSS JOIN gastos g CROSS JOIN compras c;
END;
$$;

-- get_dashboard_critical_stock (no p_user_id)
...
BEGIN
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'Not authenticated' USING ERRCODE = 'insufficient_privilege';
  END IF;
  SELECT COUNT(id) INTO v_count FROM public.products
  WHERE user_id = v_uid AND stock <= min_stock;
  RETURN COALESCE(v_count, 0);
END;
$$;

-- Admin KPIs guarded by is_admin()
...
BEGIN
  ...
  RETURN QUERY SELECT LOWER(COALESCE(event_data->>'type', 'uncategorized')) AS insight_type, COUNT(*) AS total FROM public.analytics_events WHERE event_name = 'insight_generated' AND created_at >= p_date_from AND created_at <= p_date_to GROUP BY LOWER(COALESCE(event_data->>'type', 'uncategorized')) ORDER BY total DESC;
END; $$;
