...
- [x] 1.1 Verificar el **MAX de `supabase_migrations.schema_migrations` vivo en prod** (`npx supabase migration list`, o `SELECT max(version) …`). Confirmar que el archivo nuevo se llama `20261008000001_cuenta_corriente_party_guard.sql` — `20261007000001` lo tomó `cuentas_billetera_tipo` (PR #447, mergeada mientras este propose estaba en revisión). Si prod está adelante de `20261007000001`, renumerar y anotarlo en el PR.
  ...
  > **RENUMERADO OTRA VEZ 2026-08-23 (reconciliación con #454)**: el hotfix de seguridad `revoke-internal-money-helpers` (PR #454) se mergeó después y tomó `20261010000001`, que pasó a ser el **MAX vivo de prod** (259 migraciones). Mismo motivo, tercer nombre → `supabase/migrations/20261011000001_cuenta_corriente_party_guard.sql`. Ver `design.md` Post-apply (i).
...
- [x] 2.1 **RED**: crear `supabase/tests/test_cuenta_corriente_party_guard.sql` con el molde de `test_delete_guard_ledgers.sql` […].
  ...
  > **Cleanup sin residuos (ronda 2, 2026-08-23)**: el `DO` de cleanup dejaba dos rastros por corrida que no cuelgan del `account_id` ni del `user_id` del anchor. (i) **21 filas** en `operation_idempotency` con `operation_kind = 'event_consumer'`, escritas por el dispatcher de 5.6 con un **`user_id` sentinela** (`00000000-…`), y como `event_id` **no tiene FK** quedaban apuntando a `events` ya borrados; se borran ahora **antes** del `DELETE FROM public.events`, mientras todavía se las puede resolver por cuenta. (ii) **4 filas** en `email_logs`: el FK a `auth.users` es `ON DELETE SET NULL`, así que borrar el anchor las dejaba con `user_id NULL` y el recipient sintético adentro; se borran por `user_id` **y** por `recipient` (mismo patrón que `test_tenancy_rls_role.sql` L~207). **Verificado**: purgado el arrastre histórico de las corridas del apply (112 huérfanas + 20 `email_logs`), dos corridas seguidas del gate dejan los dos conteos en **0**.
- [x] 2.2 **RED**: assert de `rpc_register_payment_received` con el `client_id` del tenant B → `P0404`. Debe **fallar hoy**.
  ...
  > **+ conteos de movimientos/cobros/eventos antes y después**: los specs no piden "falla", piden **"sin efectos parciales"**, así que el SQLSTATE solo no alcanza. Se miden `customer_account_movements`, `payments_received` y `events` del tenant A antes y después del rechazo y se exige que queden **idénticos** — que es justamente lo que el RED violaba.
- [x] 2.3 **RED**: assert de `rpc_register_payment_made` con `supplier_id` ajeno → `P0404`.
  ...
  > **+ conteos de movimientos/pagos/eventos antes y después** (espejo proveedor de 2.2): `supplier_account_movements`, `payments_made` y `events` sin cambios tras el rechazo.
- [x] 2.4 **RED**: assert de `rpc_register_supplier_charge` con `supplier_id` ajeno → `P0404`.
  ...
  > **+ conteos de cuentas/movimientos/eventos antes y después**: `supplier_accounts`, `supplier_account_movements` y `events` sin cambios. Es la más importante de las tres, porque en RED esta RPC **no fallaba en absoluto**.
...
- [x] 3.1 Crear `supabase/migrations/20261011000001_cuenta_corriente_party_guard.sql` (renumerada dos veces desde `20261008000001`, ver 1.1) con la cabecera del proyecto […]. **Sin BOM UTF-8**.
...
- [x] 3.5 **TRIANGULATE**: control positivo — cliente/proveedor **del mismo tenant** sin cuenta corriente previa: la cuenta se crea en el mismo commit y el movimiento queda con el `balance_after` esperado.
  ...
  > **Ronda 2 (2026-08-23) — 3.5b completo**: el escenario reescrito de `customer-account` promete que el cobro *"se registra con su movimiento, su fila de cobro y su evento"*, pero 3.5b sólo miraba `balance_after = 600` y `replayed = false` (o sea: el movimiento). Se agregaron los conteos **antes y después** de `payments_received` y de `events` con `event_type = 'PaymentReceived'` para el `account_id` de A, exigiendo **+1 en ambos** — mismo patrón que 3.5a con `CustomerAccountCharged`.
- [x] 3.6 **TRIANGULATE**: identificador **inexistente** → mismo `P0404`, mismo mensaje.
  ...
  > **3.6b / 3.6c**: espejo del lado **proveedor**, con el mismo texto normalizado, por las dos RPCs que el spec de `supplier-account` nombra — `rpc_register_payment_made` (3.6b) y `rpc_register_supplier_charge` (3.6c). Sin ellos el "no filtrar información entre tenants" quedaba probado sólo del lado cliente.
...
> **Barrido global (9) — nota 2026-08-23, sin task propia en el propose**: todo el grupo 3 prueba **caminos** (esta RPC, con esta parte, rechaza). Los specs de `customer-account` y `supplier-account`, en cambio, declaran el invariante a nivel **TABLA**: *"no existe ninguna fila cuyo `clients.account_id` difiera del `customer_accounts.account_id`"* (y su espejo proveedor). Un camino nuevo que nadie ejercite dejaría el invariante roto con el gate en verde, así que se agregó el assert **(9)** al final del gate: recorre `customer_accounts` y `supplier_accounts` **completas** —no sólo los tenants sintéticos— uniendo cada fila con su parte, y exige **0**. Cubre también cualquier fila que haya dejado otro gate de la corrida de CI. Es el mismo invariante que la auditoría read-only del grupo 8 mide contra prod, pero convertido en candado permanente. Anotado en la cabecera del gate dentro de la lista de tasks.
...
- [x] 4.6 **TRIANGULATE**: cliente ajeno **y** `amount = 0` → `P0400`, no `P0404`.
  ...
  > **Ronda 2 (2026-08-23) — espejo del lado PROVEEDOR**: 4.6 sólo cubría el lado cliente, pero el escenario *"Las validaciones de payload preceden al guard de parte"* del spec de `supplier-account` habla de **"un pago o un cargo manual"**. Se agregaron **4.6b** (`rpc_register_payment_made`, proveedor ajeno + `amount = 0` → `P0400`) y **4.6c** (`rpc_register_supplier_charge`, ídem → `P0400`), cada uno con su clave de idempotencia propia (`gate-ccpg-4-6b` / `gate-ccpg-4-6c`). **No son vacuos**: mutando la expectativa a `P0404` (el código que se observaría si el guard de parte corriera primero) el gate **falla** reportando el observado — `P0400` en los dos.
- [x] 4.7 **TRIANGULATE**: cobro por transferencia con cliente ajeno → `P0404` y **cero** filas en `bank_movements`.
  ...
  > **Ronda 2 (2026-08-23)**: se agregó **4.7c**, el espejo de 4.7b en `rpc_register_payment_made` (proveedor ajeno + `bank_account` inexistente → `P0412`). Sólo el **pago** tiene espejo bancario: `rpc_register_supplier_charge(p_idempotency_key, p_supplier_id, p_amount, p_reference_id)` **no recibe cuenta bancaria**, así que del cargo manual el orden sólo se puede congelar contra el importe (4.6c) — aclarado también en el spec de `supplier-account`. Mutante a `P0404` → falla reportando el observado `P0412`.
...
> 🛑 **Checkpoint OQ-2**: al escribir el apply el PO no había respondido → se implementó la **recomendación** (el tramo entra en este change). **El mismo día respondió lo contrario.**
>
...
>
> **Cómo quedó tras la reconciliación (ver `design.md` Post-apply (i))**: la migración de esta rama **no repite el `REVOKE`**. Conserva los dos `COMMENT ON FUNCTION` (5.7 — #454 no escribe comentarios, no se pisan) y agrega en su gate final una **verificación** de que ambos helpers siguen sin `EXECUTE` para `anon`/`authenticated`, con `RAISE EXCEPTION` apuntando a #454 si alguien los reabrió. **5.1, 5.2, 5.5 y 5.6 siguen `[x]` y siguen siendo nuestros**: son los candados de comportamiento, y hoy pasan **gracias a** #454 — si el hotfix se revirtiera, este gate lo grita.
...
- [x] 5.1 **RED**: con `SET LOCAL ROLE authenticated`, invocar `_pay_register_party_charge(<account_id de B>, 'customer', <client_id de B>, 1000, …)` y esperar `insufficient_privilege` (42501).
  ...
  > **+ libros del tenant B medidos**: el `42501` prueba que la llamada no entra, pero no que no haya escrito. Se cuentan `customer_accounts`, `customer_account_movements` y `events` del **tenant víctima** antes y después, exigiendo que queden **sin cambios** — que es exactamente lo que el RED violaba (1 `customer_accounts` nueva en B).
- [x] 5.2 **RED**: espejo con `_journal_post_from_event(<fila events forjada con account_id de B>)` → `insufficient_privilege`.
  ...
  > **+ libros del tenant B medidos**: se cuentan los `journal_entries` de B antes y después y se exige **sin cambios**, además de verificar que el evento forjado **no** tiene asiento asociado (`source_event_id`). Igual que en 5.1, el SQLSTATE solo no alcanzaba.
- [x] 5.3 **GREEN**: `REVOKE ALL ON FUNCTION public._pay_register_party_charge(uuid, text, uuid, numeric, uuid, uuid) FROM PUBLIC, anon, authenticated;` — sin tocar el cuerpo.
  > **HECHO EN #454, no acá** (SUPERSEDED, ver la nota del grupo): el `REVOKE` vive en `supabase/migrations/20261010000001_revoke_internal_money_helpers.sql`, ya mergeado en `main`. De esta rama quedan el `COMMENT` (5.7) y la verificación del gate. El cuerpo no se toca en ninguno de los dos archivos.
- [x] 5.4 **GREEN**: `REVOKE ALL ON FUNCTION public._journal_post_from_event(public.events) FROM PUBLIC, anon, authenticated;`.
  > **HECHO EN #454, no acá** (SUPERSEDED): mismo archivo `20261010000001_revoke_internal_money_helpers.sql`. Restaura el estado de `20260803000001` L517 perdido en `20261001000001` L1914.
...
- [x] 5.7 `COMMENT ON FUNCTION` de ambos helpers: dejar escrito **por qué** no llevan `GRANT`.
  > **Evidencia**: ambos comentarios abren con **"SIN GRANT — NO AGREGAR UNO"**, explican el vector concreto (account_id por parámetro / fila de `events` completa aceptada como JSON por PostgREST), citan la verificación empírica en local, nombran al único caller legítimo y remiten a los chequeos (3) y (4) del gate. **Se conservan tras la reconciliación**: #454 no escribe ningún `COMMENT` (verificado), así que no hay pisada; sólo se corrigió la atribución del `REVOKE` (es de #454, no de esta rama).
...
>
> **Hecho en la reconciliación**: el chequeo de esta rama quedó numerado **(4)** y el de #454 quedó intacto como **(3)**. Los dos conviven en el mismo `DO $$`, con la cabecera del archivo describiendo (1)(2)(3)(4) y el `RAISE NOTICE` final nombrando los cuatro. La evidencia de abajo se dejó con la redacción original de cuando el chequeo era el (3), con la aclaración correspondiente donde importa.
...
- [x] 6.1 Agregar la migración como **último eslabón** de la cadena del step "Verify G1/G4 migrations are idempotent on reapply", con su comentario.
  > **Evidencia**: agregado tras `20261006000001`. El comentario explica algo que **no** estaba previsto en el propose y que se descubrió al reproducir la cadena en local: mi migración no va última sólo por ser la más nueva, sino porque **el reapply de `20261001000001` (L137) y `20261004000001` (L1778) vuelve a ejecutar el bloque REVOKE+GRANT que le devuelve el GRANT a `authenticated` a los dos helpers** — o sea, reabre el agujero.
  > **Post-reconciliación**: la cadena termina ahora con DOS eslabones nuevos, en este orden — `20261010000001_revoke_internal_money_helpers.sql` (#454, que hace el `REVOKE` de los dos helpers de dinero) y `20261011000001_cuenta_corriente_party_guard.sql` (el nuestro, que revoca los `c30_get_or_create_*` y **verifica** que los de #454 sigan cerrados). El nuestro sigue siendo el último, y ahora hay una segunda razón para que lo sea: su gate audita el estado que deja el eslabón anterior.
- [x] 6.2 Descubrir la lista **real** de offenders del chequeo nuevo contra la DB de CI (post-migraciones) **y** contra prod. Comparar y anotar las diferencias.
  > **Evidencia**:
  > | entorno | offenders del chequeo amplio (hoy el **(4)**) |
  > |---|---|
  > | **prod**, hoy (pre-merge de #454) | 3: `_c29_confirm_order_core`, `_journal_post_from_event`, `_pay_register_party_charge` |
  > | **local**, post-migración | **1**: `_c29_confirm_order_core` |
  > | **prod**, esperado post-merge | 1 (verificación 9.4) |
  > Diferencia local/prod adicional (gotcha #432): `c30_get_or_create_*` y `c30_register_*_account_movement` son anon-executable en prod y no en local — no caen en el chequeo amplio porque son SECURITY INVOKER, pero la migración los revoca igual.
- [x] 6.3 **RED**: agregar el chequeo amplio (numerado **(4)** tras la reconciliación) **sin** poblar la allowlist. Verificar que falla y lista exactamente los offenders de 6.2.
  > **Evidencia**: corrida con la allowlist vacía (`ARRAY[]::text[]`) → falla listando **exactamente** `public._c29_confirm_order_core(...)`, una sola línea.
- [x] 6.4 **GREEN [OQ-3]**: poblar la allowlist con los offenders preexistentes que este change no revoca, con su comentario. `_pay_register_party_charge` y `_journal_post_from_event` **NO** entran.
  > **Evidencia**: allowlist de **1 entrada** (`_c29_confirm_order_core`, que **sí es alcanzable cross-tenant** vía `p_cash_session_id` — ver h1 en Post-apply del `design.md`; se allowlistea igual porque el hueco es alcanzable por sus wrappers públicos `rpc_quick_sale`/`rpc_confirm_sales_order`, fuera del filtro de nombre del chequeo → revocar el helper no cierra nada y arriesga el POS). Comentario negativo explícito de que los dos revocados no deben entrar — reforzado tras la reconciliación: ahora además los cubre la lista cerrada del chequeo **(3)** de #454, que no admite allowlist, así que una excepción acá apagaría el (4) y dejaría gritando solo al (3).
...
- [x] 6.5 Documentar en el encabezado del gate la regla de mantenimiento, el mecanismo que produjo el hallazgo y el gotcha prod≠local.
  > **Evidencia**: cabecera ampliada con (i) por qué existe el chequeo amplio y qué punto ciego cubre, (ii) el mecanismo — patrón uniforme REVOKE+GRANT de `20261001000001` L137/L1914, (iii) por qué filtra por convención de nombre y no por "todas las SECURITY DEFINER" (~76 `rpc_*` legítimas), (iv) el gotcha #432 con el caso real medido, (v) la regla de mantenimiento (achicar siempre válido; agregar exige justificación en el PR).
  > **Post-reconciliación**: la cabecera se reescribió para describir los **cuatro** chequeos sin duplicar párrafos ni contradecir la de #454 — se conserva íntegro su párrafo de `v_internal_only_fns`, se agrega un párrafo nuevo que explica por qué (3) y (4) son redundantes **a propósito** (lista cerrada nominal vs. barrido por convención), y la regla de mantenimiento distingue las dos allowlists del chequeo (3), que no tiene ninguna.
- [x] 6.6 **TRIANGULATE**: verificar que el chequeo amplio **no** reporta las `rpc_*` públicas (control negativo).
  > **Evidencia**: **0** de las **76** funciones `rpc_*` SECURITY DEFINER concedidas a `authenticated` caen en su filtro de nombre. La allowlist se mantiene en 1 entrada — legible, y por lo tanto un gate encendido.
...
- [x] 6.8 Correr los siete gates del baseline 1.3 **después** de la migración y verificar que siguen verdes. Verificar además, con un `supabase db reset` limpio, que la corrida completa en el orden exacto de CI pasa.
  ...
  > **Control positivo del chequeo amplio (entonces (3))**: en una transacción, `GRANT EXECUTE … TO authenticated` sobre `_pay_register_party_charge` → el gate **falla** listando exactamente esa firma; tras el `ROLLBACK`, `has_function_privilege` vuelve a `false`. El gate no pasa en falso.
  >
  > **Evidencia (reconciliación con #454, 2026-08-23)**: `npx supabase db reset` limpio → **260 migraciones, `MAX = 20261011000001`**, con `20261010000001 revoke_internal_money_helpers` (#454) y `20261011000001 cuenta_corriente_party_guard` aplicadas en ese orden. Cadena de reapply reproducida en el orden exacto del workflow, **13 eslabones** (los 11 previos + los 2 nuevos), con los **mismos dos** fallos tolerados que el YAML documenta y sólo esos (`20260928000001` y `20261002000001`, cada uno por su gate anti-overload). Al final de la cadena, `has_function_privilege('authenticated', …, 'EXECUTE')` = **`false`** en los cuatro: `_pay_register_party_charge`, `_journal_post_from_event`, `c30_get_or_create_customer_account`, `c30_get_or_create_supplier_account`. Gates re-corridos después de la cadena: `test_function_acl_gate`, `test_errcode_5char_gate`, `test_idempotency` y `test_cuenta_corriente_party_guard` — los 4 **VERDES**. El gate de este change corre **dos veces seguidas en verde** (19 asserts `PASS` cada vez) más una tercera pasada, con `cleanup completo (2 anchors)` en todas: sin residuos. Suite backend: **1604 passed / 3 skipped** (#455 no agrega tests de pytest).
  > **Controles positivos de los dos chequeos ACL, medidos por separado** (cada uno en `BEGIN … ROLLBACK`, con `has_function_privilege` de vuelta en `false` después):
  > - **(3)**, lista cerrada de #454: `GRANT … TO authenticated` sobre `_pay_register_party_charge` → `ACL GATE (3) FAILED` listando exactamente esa firma.
  > - **(4)**, barrido por nombre de esta rama: `GRANT … TO authenticated` sobre `_pay_register_operation_bank_movement(…)` → `ACL GATE (4) FAILED` listando exactamente esa firma. **Nota**: el control de (4) **no puede** hacerse con `c30_get_or_create_customer_account` —el candidato obvio— porque esos dos son `SECURITY INVOKER` y el chequeo (4) filtra por `prosecdef`; se usó un helper interno `SECURITY DEFINER` no allowlisteado y ajeno a `v_internal_only_fns`, que es lo que el chequeo realmente vigila.
  > - **Verificación (b) de la migración**: `GRANT … TO authenticated` sobre `_journal_post_from_event` y después aplicar `20261011000001` → la migración **aborta** con `cuenta-corriente-party-guard ACL: los helpers de dinero revocados por 20261010000001_revoke_internal_money_helpers.sql (PR #454) volvieron a ser ejecutables…`. La verificación nueva no es decorativa.
  >
  > `ACL GATE OK: sin triggers SECURITY DEFINER expuestos; anon-executable dentro de la allowlist (5 firmas permitidas); 3 helpers solo-DEFINER intactos; helpers internos authenticated-executable dentro de su allowlist (1 firmas permitidas)`.
