...
- **Candidatos que deja**: migrar las dos copias JS de "top productos" (`supabase/functions/ai-insights`, `frontend/lib/ai/buildBusinessSnapshot.ts`) a `rpc_product_ranking`; migrar los 3 reportes existentes a `components/charts/`; filtro de canal sin superficie (`p_canal` existe en RPCs y API, ninguna pantalla lo expone); alinear `ai-rentabilidad` para no incrementar el contador con insight vacío (hoy lo hace; `ai-estadisticas` no); el reporte completo XLSX no incluye una hoja de ranking (fuera de alcance, sería la 7ª hoja con los mismos parámetros que hoy no tiene cómo elegir); al archivar, reconciliar la spec `product-ranking` ("ranking por categoría") con la forma real (5ª dimensión de `rpc_sales_breakdown`); 12.6 (verificar en prod `MAX(version) = 20261026000001`, ACLs de `rpc_product_sales_evolution`, el CHECK de `export_logs` ampliado y el conteo 218 → 0 de `last_sale_date` corridos), un humo real de `ai-estadisticas` con la clave de prod y 14.3 (demo del PO) tras el merge.
- **Leer antes**: `openspec/changes/estadisticas-ventas/design.md` (D1-D15, OQs firmadas y notas de implementación de E1, E2 y E3), las cabeceras de `supabase/migrations/20261024000001_estadisticas_ventas_e1.sql`, `20261025000001_estadisticas_ventas_e2.sql` y `20261026000001_estadisticas_ventas_e3.sql` (checkpoints vivos + desvíos declarados).
