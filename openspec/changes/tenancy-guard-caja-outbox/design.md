...
- **Capa 1, en `_c29_confirm_order_core`: invariante de SUCURSAL.** Espejo del `cash_optin_requires_open_session` del formulario: `cs.status = 'open' AND cb.branch_id = v_gate_branch` → `P0422`. Es la **única** capa que puede expresar esto, porque `v_gate_branch` —la sucursal efectiva de la venta— sólo existe dentro del core. Cierra el POS por sus dos wrappers de una vez.

  > **El guard es AUTOSUFICIENTE — una diferencia deliberada contra el predicado copiado (segunda ronda adversarial, 2026-08-24).** El SELECT del guard JOINea `branches` y exige además `b.account_id = v_account_id`. Motivo medido: en el formulario la `v_gate_branch` ya viene validada como perteneciente al tenant **antes** del bloque, y en el core **no** — la sucursal de la orden la elige el payload en el camino de `rpc_quick_sale` (`COALESCE(p_branch_id, c26_default_branch(...))` va al `INSERT` en `sales_orders` sin chequeo de cuenta, y `pg_constraint` sobre esa tabla no tiene CHECK ni FK que ligue `branch_id` con `account_id`), mientras que la validación de tenencia de la sucursal (`AND account_id = v_account_id` del `branch_not_found`) corre unas líneas **más abajo**. Sin la cláusula nueva, mandar la sucursal de la víctima junto con su sesión de caja **satisface** el guard, y lo único que frena la escritura pasa a ser un chequeo ajeno a este change que **ningún test congela** — borrar hoy ese `AND` no hace fallar ningún assert y reabre h1 por ese camino. Medido: ese payload rebotaba con `P0404 branch_not_found`, no con `P0422`. Con la cláusula, el guard establece por sí solo los dos invariantes. **No se movió de posición**: bajarlo debajo del `branch_closed` habría hecho que el `branch_not_found` se evaluara primero —o sea, mismo `P0404`— además de reanclar el candado de posición (3.2). Candados nuevos: asserts (2.2c) y (2.3b) del gate.
...
| gate embebido de `20260804000003` §(b) | anchor sintético dentro de la migración | **pasa en DB fresca**; sólo degrada si se re-aplica con el guard puesto — ver abajo |
...
- ~~**Sí apaga en silencio ese gate** en toda DB fresca.~~ **CORREGIDO CON MEDICIÓN (2026-08-24, tasks 3.6 y 9.3).** No lo apaga: en un `supabase db reset` las migraciones se aplican por timestamp, así que `20260804000003` corre mucho antes de que exista `20261013000001` y su gate (b) se ejercita **con el helper todavía sin el backstop**, y pasa. Se probó aislando el bloque literal contra los dos cuerpos: con el baseline pre-guard asertó 1500 / 1300 / 1600, con el guard puesto degradó a `NOTICE` por `P0401`. Además `20260804000003` **no** está en la cadena de reapply del workflow. La degradación sólo aparece si alguien **re-aplica** esa migración a mano con el guard ya puesto — caso para el cual el ERRCODE sigue siendo la diferencia entre degradar y abortar. Consecuencia: la réplica de la aserción en el gate nuevo (+500 / −200 / +300 → 1500 / 1300 / 1600) es cobertura **ADICIONAL**, no un reemplazo. Task 3.7.
...
El guard nuevo va **inmediatamente después de `cash_requires_session`**, junto a las demás validaciones de payload y **antes de la primera escritura**. Razones: (i) **coherencia con el propio core**, que es donde ya viven todas sus validaciones de payload; (ii) un `RAISE` posterior a las escrituras revierte igual —la transacción es una sola— pero desperdicia trabajo y hace el error más difícil de leer; (iii) congela el orden como invariante testeable: un test puede verificar que con `p_cash_session_id` ajeno **y** un payload inválido por otra razón, gana el error de payload.

> **Corrección de la razón (i) — revisión adversarial, 2026-08-24.** La versión original de este párrafo decía *"coherencia con el formulario, donde el bloque `cash_optin_*` está exactamente ahí"*. **Es falso**, y lo desmiente el baseline que este mismo change capturó: en `baseline/rpc_create_sale_operation_v2.sql` la idempotencia está en L113-137 y el bloque `cash_optin_*` en L274-292, o sea **después** del short-circuit de replay (su propia cabecera lo dice: *"bloque cash_optin_requires_open_session, P0422, al final del cuerpo"*). En el core es al revés: `INSERT INTO operation_idempotency` está en la L162 del baseline y el bloque de validación de payload, entero, antes.
>
> **Consecuencia declarada, y decisión de conservar la posición.** Un reintento con la misma `Idempotency-Key` deja de devolver el replay si la sesión se cerró en el medio, y devuelve `P0422` (assert 3.12 del gate). Eso **no es una propiedad nueva**: el cuerpo vivo de prod ya revalidaba ahí `is_account_writer`, `payment_method_inactive` (P0400), `cash_requires_session` (P0400) y `branch_closed` (P0422). Medido contra el baseline **pre-guard**: el replay con la sucursal cerrada entremedio **ya rebotaba con P0422 antes de este change**. El guard hereda la propiedad; no la inaugura. Mover sólo este guard partiría el bloque en dos y dejaría el contrato roto igual para las otras tres precondiciones — media reparación es peor. **Restaurar el contrato de `Idempotency-Key` del POS (mover el bloque ENTERO después del replay, como lo tiene el formulario) es un change propio**, y arrastra el reanclado del candado 3.2 del gate, que hoy exige `guard < INSERT INTO public.operation_idempotency`. Riesgo real acotado: `mutations.retry = false` en el query provider y el POS reconstruye el payload en cada submit con la sesión abierta viva, así que la UI no produce el caso; y la clave sólo se resetea en el submit exitoso, con lo cual un `P0422` en el reintento **no puede** inducir una venta duplicada.

> **Paridad con el formulario: es PARCIAL, a propósito (2026-08-24).** Sobre el mismo `p_cash_session_id` el formulario aplica **tres** guards y este change replica **uno**, el de tenencia (`cash_optin_requires_open_session`). No se replican `cash_optin_requires_cash_kind` ni `cash_optin_requires_today`. Motivo: son higiene de input, no tenencia. Con `kind` no-`cash` el id se descarta sin tocar caja (el consumo vive dentro de `IF v_kind = 'cash'`) y el rechazo por tenencia funciona igual —verificado: POS con `transfer` + sesión de otro tenant → `P0422`, assert 2.6—; y la fecha del POS la fija `reporting_local_today()` en el propio `INSERT`, así que `cash_optin_requires_today` se cumple por construcción. Replicarlos **agrandaría** el BREAKING de un change CRÍTICO sobre payloads que ninguna superficie del producto genera (el POS manda `cash_session_id` sólo con `resolvedKind === "cash"`), a cambio de cero seguridad. Queda como **candidato**: *paridad completa de guards de `p_cash_session_id` entre POS y formulario*, junto con el reordenado del replay — los dos tocan el mismo bloque y conviene hacerlos juntos.
...
*Recomendación: **las dos** (D1).* La capa 2 cuesta un `SELECT` de cuatro líneas en una función de 2 288 caracteres, no cambia la firma, y es lo único que cubre callers futuros. El costo que se le atribuía —el gate embebido de `20260804000003` degradado a `NOTICE`— **resultó no existir en CI** al medirlo (task 3.6): ese gate corre por timestamp antes de que exista el guard y no está en la cadena de reapply, así que sigue ejercitándose entero. Sólo degrada si alguien re-aplica esa migración a mano con el guard puesto, y para ese caso la elección de `P0401` sobre `P0001` es lo que evita que aborte. La réplica de la aserción (task 3.7) queda como cobertura adicional.
