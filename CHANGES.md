...
**Change `seguros-perfil-asesor`** 🔨 **PROPUESTA + APPLY COMPLETO 2026-09-01** (artefactos en `openspec/changes/seguros-perfil-asesor/`; implementación en la rama `opsx/seguros-perfil-asesor-apply` — **PR y archive pendientes**; governance MEDIO). Publica el **acuerdo comercial real** con Julián Dupás (Productor Asesor de Seguros, Mat. N.º 98506): `/seguros` está en producción desde 2026-03 pero `community.seguros` tenía **0 filas** (verificado en prod el 2026-09-01) porque su modelo de 4 campos de texto plano (`title`/`description`/`coverage`/`price`) no podía contar lo que el material del partner realmente es — 3 líneas de servicio, 4 pilares desarrollados, 14 ciudades de alcance, matrícula de PAS y contacto estructurado. El PO firmó **Opción B: perfil de asesor** (no ficha simple, no destacado+grilla forzada).
...
- **Segundo hallazgo real**: tipar `AdminStats.timeSeries` (retirando el `useState<any>` de `/admin/seguros`, regla del proyecto) sacó a la luz que el chart "Evolución temporal" de ese panel viene **roto desde que se escribió** — `TimeSeriesLinesChart` espera `{period: string parseable como Date, ...}` pero `processTimeSeries` entrega `{name: "Ene".."Dic", value}`, invisible mientras `stats` fue `any`. Fuera de alcance de este change arreglarlo; se dejó un adapter local para que compile sin cambiar el comportamiento (roto) preexistente y se registró como candidato aparte.
...
- **6 OQs con recomendación, ninguna bloqueante** (D7/D10 hacen que el diseño degrade limpio si no se responden): OQ-1 vía de contacto principal (WhatsApp confirmado por el PO el mismo día — ver seed), OQ-2 sin gating de plan (recomendación aplicada: abierto a todos), OQ-3 matrícula y leyenda regulatoria (la matrícula personal **sí estaba** en el material, corrección al brief original; la del broker `argbroker.com.ar` queda sin resolver — `license_authority` sembrado vacío), OQ-4 disclaimer (aprobado por el PO el mismo día — ver seed), OQ-5 alcance de publicación (sólo dentro de la app, aplicado), OQ-6 foto (resuelta: sí había).
- **Sin backfill** (tabla vacía) y **sin cambios de RLS**. Migración `20261017000001_seguros_perfil_asesor.sql`.
- **Leer antes**: `openspec/changes/seguros-perfil-asesor/design.md` (D1-D11 + las 6 OQs), `docs/Julian_Dupas_PAS_v5_260814_174911.pdf` (material fuente).
