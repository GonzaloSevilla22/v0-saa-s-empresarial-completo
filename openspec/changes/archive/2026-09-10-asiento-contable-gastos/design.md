...
**(b) `/reportes/libro-diario`** — pantalla nueva en el grupo **Inteligencia** del `frontend/components/app-sidebar.tsx` (donde ya viven "Centros de costo" y "Formas de pago", `app-sidebar.tsx:83-87`; el propose se equivocó de nombre de grupo — no existe ningún grupo "Reportes" en el sidebar, corregido por el revisor en la ronda 2), sin gate de plan, por el mismo criterio que esos dos: es la lectura de datos que el usuario ya generó. Lista de asientos con fecha, tipo de documento, estado, y sus líneas de débito/crédito expandibles; filtros por rango de fechas, tipo de documento y estado. Ambos temas y ambos anchos, verificados antes del merge.
...
### D14 — Corrección de findings (revisor, ronda 1, 2026-09-10)

**[blocker] El lookup del "asiento vigente" en `ExpenseAdjusted`/`ExpenseDeleted` no excluía contra-asientos.** El molde de `SaleOperationAdjusted` del que D6 se copia byte a byte tiene el mismo defecto latente (hoy sin daño histórico en prod porque la venta por formulario no se edita tan libremente como el gasto sin dinero posteado), pero D6 lo vuelve *alcanzable de rutina*: un gasto sin movimiento de caja ni bancario es editable a voluntad, así que la segunda edición del mismo gasto encuentra DOS filas `status='posted'` (el asiento nuevo de la 1ª edición y su propio contra-asiento) y el `LIMIT 1` sin desempate elige una arbitraria. **Fix**: `AND reversal_of IS NULL` en los dos lookups + `ORDER BY posted_at DESC, created_at DESC` por robustez — con el filtro alcanza (hay una sola fila válida en todo momento), el `ORDER BY` es defensivo. Cubierto por el bloque (F) ampliado con una segunda edición y por el bloque nuevo (F2), alta→edición→borrado.

*No se generaliza el fix a las ramas de venta/compra/cobro que comparten el mismo molde* — están fuera del alcance declarado de este change (edita `expenses`, no `sales`/`purchases`/`payments_*`) y hoy sin daño medido en prod; queda anotado como candidato en `CHANGES.md`, no corregido acá.

**[major] `rpc_update_expense` derivaba el `kind` del crédito de la forma de pago DECLARADA, no de lo que realmente movió dinero.** D4 ya dejó escrito el principio para el alta ("el asiento nombra la cuenta del libro que efectivamente se movió") pero la edición no lo cumplía: `rpc_update_expense` nunca llama a `_pay_register_operation_bank_movement` (la edición no mueve dinero, D6), así que cambiar la forma de pago a un `kind` bancario en la edición acreditaría `1110 Banco` sin ningún `bank_movement` real que lo respalde — un descuadre de conciliación. **Fix**: `v_final_kind` en `ExpenseAdjusted` se deriva de la EXISTENCIA de `bank_movements` para el gasto, nunca del `payment_method` elegido. En la práctica esa rama resuelve siempre a "sin bank_movement" (el guard `P0423` de arriba en la misma función ya bloquea la edición en cuanto existe uno), así que el crédito de toda edición cae a `1100 Caja` — consistente con D6: sólo se puede editar un gasto que, por construcción, nunca movió banco. El `IF EXISTS` se deja igual (no un `v_final_kind := NULL` directo) para que el derivado siga siendo correcto sin tocarlo si el guard de arriba cambia alguna vez. Cubierto por el bloque nuevo (F3).

### D15 — Corrección de findings (revisor, ronda 2, 2026-09-10)

**[major] El delta de spec prescribía la clave de búsqueda del "asiento vigente" SIN `reversal_of IS NULL`.** El fix [blocker] de D14 (arriba) corrigió el CÓDIGO (`AND reversal_of IS NULL` en los dos lookups de la migración) pero el propose nunca actualizó `openspec/changes/asiento-contable-gastos/specs/journal-entry/spec.md`: las dos requirements (`ExpenseAdjusted`/`ExpenseDeleted`) seguían enumerando la clave de 4 columnas sin el quinto predicado. Al archivar, ese delta se funde en la spec principal y se vuelve doctrina permanente — una reimplementación futura que la siguiera al pie de la letra reproduciría el mismo doble conteo que el blocker de la ronda 1 encontró en el código. **Fix**: puramente documental — se agrega `reversal_of IS NULL` a la tupla en las dos requirements, un SHALL normativo ("a lo sumo un asiento vigente por gasto en todo momento") y dos scenarios nuevos (segunda edición; alta→edición→borrado) que fijan en la spec lo que los bloques (F) y (F2) del gate ya verifican en el código.

**[minor, residual documentado — decisión (b), no corregido en código] Editar la forma de pago de un gasto a un `kind` bancario sin `bank_movements` reales deja el documento diciendo "Transferencia" mientras el asiento acredita `1100 Caja`.** Es consecuencia directa del fix [major] de D14: ese fix hace que el asiento sea el más HONESTO de los tres artefactos (documento, `bank_movements`, asiento) porque nunca acredita `1110 Banco` sin un movimiento bancario real que lo respalde — pero la contradicción entre "el documento dice Transferencia" y "el asiento dice Caja" queda visible por primera vez en `/reportes/libro-diario`, la pantalla que este mismo change agrega. El revisor ofreció dos opciones: (a) rechazar en `rpc_update_expense` el cambio a un `kind` bancario cuando el gasto no tiene `bank_movements` (`P0400` nuevo), o (b) documentarlo y dejarlo para el humo real del PO. Se elige **(b)**: es una decisión de diseño ya firmada en D14/D6 (la edición de un gasto nunca mueve dinero), no un bug de este change; agregar un `ERRCODE` y un camino de rechazo nuevos infla la superficie de una corrección de findings sin que el revisor lo haya calificado de correctness/seguridad — el asiento sigue siendo contablemente correcto (no hay descuadre: no hay `bank_movement`, no se acredita `1110`). Queda anotado también como candidato en `CHANGES.md` para que el PO lo confirme en la task 14.2 (humo real).

**[nit] Los dos predicados D7 no aprovechaban `idx_events_aggregate` por prefijo.** El índice (creado en D14 arriba) lidera por `(aggregate_type, aggregate_id)`, pero los `EXISTS` de `rpc_update_expense`/`rpc_delete_expense` filtraban sólo por `event_type` + `aggregate_id` + `account_id`, sin `aggregate_type` — `EXPLAIN` confirmó `Index Cond` sólo sobre la segunda columna del índice (recorrido completo, no búsqueda por prefijo). **Fix**: se suma `AND aggregate_type = 'Expense'` a los dos predicados — son eventos de gasto por construcción, no cambia la semántica de "existe el alta" — y ahora el índice calza por columna líder. Costo hoy despreciable (883 filas en prod), pero corrige la forma antes de que el volumen importe.

**[nit] `design.md` §D9 (arriba, corregido inline) decía que la pantalla nueva cuelga del grupo "Reportes" del sidebar — ese grupo no existe.** El grupo real (donde viven "Centros de costo" y "Formas de pago", y donde la implementación efectivamente montó "Libro diario") es **Inteligencia**. Corregido en el propio D9.

**[nit] El comentario del bloque (3.10h) de `test_gastos_forma_pago.sql` prometía verificar el mapeo a `1110 Banco` pero sólo contaba asientos.** Ver el bloque reescrito en el propio archivo — ahora assertea las líneas `5300:debit`/`1110:credit` en vez de sólo describirlas.

**[nit, informativo — sin acción de código] La línea de la cadena de reaplicación de idempotencia en `KPI_Validation.yml` quedó insertada después de `20261040000001`, pero `origin/main` ya tiene `20261041000001`/`20261042000001` después de esa posición.** No se reordena en este apply — el propio comentario del change (y el brief del orquestador) ya anticipan que el rebase de esta rama contra `main` es responsabilidad del orquestador al reordenar; el delta medido no tiene conflictos semánticos (ningún archivo de main tocado por este change choca con `_EXPENSE_PROJECTION`/`ExpenseOut`). Queda para el rebase, no para esta corrección de findings.
...
## Resueltas por sign-off del PO (apply, 2026-09-10)

Las seis OQs se resolvieron por su recomendación (sign-off general del PO al orquestador, "si a todo"):

- **OQ-1 (a)**: gasto sin forma de pago → `1100 Caja`, nunca `2100 Proveedores`.
- **OQ-2 (a)**: `kind='cash'` sin opt-in de caja acredita `1100` igual — la adhesión gobierna el arqueo, no la cuenta contable.
- **OQ-3 (a)**: sin backfill de los gastos históricos. Medición del checkpoint 1.7 sobre la mesa antes de firmar: 208 gastos, 176 sin forma de pago (84,6%), 16 `cash`, 13 `wallet`, 3 `transfer`. Aditivo en un change aparte si el PO lo pide más adelante, con eventos `source='backfill'`, nunca `INSERT` directo a `journal_entries`.
- **OQ-4 (a)**: la pantalla del libro diario **entra** en este change — `/reportes/libro-diario`, con filtros y líneas expandibles.
- **OQ-5 (a)**: el estado contable del gasto es visible a **todos** los miembros de la cuenta (coherencia con la RLS y el endpoint ya vivos).
- **OQ-6 (a) con salida a (b), resuelto sin necesidad de (b)**: `test_cobranzas_reverso.sql` corrió limpio contra la base sin migrar (12/12 bloques, EXIT=0) antes de cablearlo — no apareció ningún rojo preexistente que reportar ni arreglar.
