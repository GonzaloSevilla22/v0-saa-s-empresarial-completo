...
-- MAX(supabase_migrations.schema_migrations.version) al momento de la
-- captura: 20261004000002 (confirmado, sin drift respecto del design.md).
--
-- Gate de integridad de función (regla del proyecto, 5 incidentes previos):
-- la reescritura de esta función para las ramas SaleOperationDeleted /
-- PurchaseDeleted parte de ESTE archivo, no de ningún archivo de migración
-- anterior. Las 7 ramas existentes (SaleConfirmed con y sin desglose fiscal,
-- PurchaseCreated, SaleOperationCreated, SaleOperationAdjusted,
-- PaymentReceived, PaymentMade, CreditNoteIssued) se preservan byte a byte.
