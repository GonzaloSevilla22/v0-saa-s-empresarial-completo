# enterprise-contact-tier Specification

## Purpose

El tier "Empresa" como oferta de contacto comercial directo: presencia y contenido de la card en las dos superficies de precios (landing pública y `/planes`), CTA a WhatsApp con mensaje prellenado propio del tier, degradación explícita sin número de contacto configurado, presentación adaptada a la superficie que lo contiene, y la frontera dura con el motor de facturación — no es un `Plan`, no tiene precio, no entra a checkout, no participa de gating ni de los KPIs de negocio (MRR).

Origen: change `plan-empresa-contacto` (2026-08-14). 100% frontend, sin migraciones ni backend.
