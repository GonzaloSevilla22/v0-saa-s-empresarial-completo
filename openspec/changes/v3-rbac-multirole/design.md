...
- `require_role(auth, [...])` — **59 llamadas en `backend/services/*.py`, las 59 con la lista `["user","admin"]`**. Es el rol de **plataforma** (`profiles.role`), un espacio de nombres distinto del rol de tenant (declarado normativo en `authz-token-claims`). **Ninguna de las 59 entra en el alcance de este change** — el explore las contaba dentro de "65+4 call sites" a migrar; la medición las saca del alcance.
...
- **Migrar los 59 `require_role(auth, ["user","admin"])`.** Son rol de plataforma; tocarlos sería mezclar dos espacios de nombres que `authz-token-claims` declara separados.
