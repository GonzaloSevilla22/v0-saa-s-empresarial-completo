...
Vive en el service (`_guard_base_unit_change`) sobre una lectura del repository (`has_stock_or_movements`, filtrada por `account_id`). Asignar la unidad a un producto que no tenía, aunque ya tenga stock, se permite a propósito: es la vía para que los productos que se venden por kilo (OQ-1) declaren su base, y el stock que hasta ahora estaba "en unidades sin nombre" pasa a leerse en esa unidad. Si el PO elige (b) o (c), este guard se reemplaza. Tests: `backend/tests/test_ventas_unidades_conversion_base_unit_lock.py`.
