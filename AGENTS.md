...
17. **`qa-integral-modulos`** 🔨 **PROPUESTA 2026-08-31** (artefactos en `openspec/changes/qa-integral-modulos/`; **apply pendiente**; governance LOW/MEDIUM por grupo — MEDIUM lo que toca `ui/popover.tsx`, `ui/sidebar.tsx`, el sistema de toast, el guard de proveedor, billing y las 3 reescrituras de RPC). Cierra por **causa raíz** los 27 hallazgos confirmados del QA integral del 2026-08-30 (informe copiado al change: `qa/INFORME.md` — fuente de verdad) + 3 defectos de backend de la misma corrida. **Dos componentes compartidos explican 17 de los 27 síntomas**: el popover portalizado fuera del shard de scroll del modal (bug del PO: ningún desplegable scrollea dentro de un modal) y el `<main>` del shell sin `min-w-0` (12 pantallas desbordan en móvil escondiendo botones primarios). 16 grupos G1-G16, capability nueva `responsive-shell` + 6 modificadas, migración `20261016000001` (3 RPCs bajo regla de integridad de función), 3 OQs no bloqueantes con recomendación. **Absorbe el candidato del desborde de `/gastos`** anotado abajo. Detalle en `CHANGES.md`.
