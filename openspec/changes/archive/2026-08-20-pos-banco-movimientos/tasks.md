...
- [x] 11.2 Verificación post-deploy **read-only** vía MCP (post-merge, `MAX(version)=20261002000001`): las 8 funciones nuevas/modificadas con `p_bank_account_id` tienen exactamente 1 firma cada una (sin `42725`), `payment_methods.bank_account_id` existe (uuid, nullable), ACLs correctas (helpers `_pay_resolve_bank_account`/`_pay_register_operation_bank_movement` sin `EXECUTE` para `anon`/`authenticated`; RPCs con `authenticated` y sin `anon`), `bank_movements = 0` (esperado — nadie configuró un destino todavía, deploy es un no-op funcional hasta que el PO opte, tal como documenta el design). **No se ejecutó la venta de prueba en vivo** (asignar un destino + registrar una venta son escrituras) — fuera del alcance permitido de esta sesión (Prod: SOLO SELECTs). La cadena ya fue ejercitada con escrituras reales en `test_pos_banco_movimientos.sql` (sesión sintética, CI) — verificación live en prod queda para el PO.
...
- [x] 11.4 `CHANGES.md` actualizado: entrada completa de `pos-banco-movimientos`, OQ-A cerrada, saga de pagos declarada completa, candidatos nuevos (`delete-guard-ledgers` destacado, `tarjeta-settlement-t-n`, `gastos-forma-pago`). Decisiones (D1 inmediato-vs-esperado, D2 regla de resolución, D9 UX) y OQ-1..4 registradas en engram (`opsx/pos-banco-movimientos/apply`).
