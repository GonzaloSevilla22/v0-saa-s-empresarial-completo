...
Guard: require_account_role(conn, auth, ["owner", "admin"]) — mismo criterio
de rol de TENANT que cost_centers.py/payment_methods.py/product_categories.py
(D9 v31-authz-token-hook), en línea con el guard is_account_writer (también
owner/admin) que las RPCs re-verifican en la DB (defensa en profundidad).
