...
- `asiento-venta-formulario` — la venta del formulario (241+ operaciones en prod, cifra creciente) no emite `SaleConfirmed` y por lo tanto no tiene asiento contable. Cerrarlo requiere un `event_type` de forma operación (no `sales_orders`), una rama nueva en `_journal_post_from_event` con su propio lookup de neto/IVA, y backfill de las operaciones históricas — fabricar una `sales_orders` sintética por cada una se descartó (contamina el agregado del POS, rompe el conteo de 120 que hoy es invariante verificable).
