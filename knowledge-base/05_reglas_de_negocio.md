...
`stock_movements` es una tabla de solo-inserción. Ningún registro puede ser modificado ni eliminado por usuarios (policies `UPDATE`/`DELETE` con `qual=false`). Las correcciones se realizan mediante un movimiento nuevo — de tipo `adjustment` para ajustes manuales, o de tipo `sale_return`/`purchase_return` como **contramovimiento** cuando se elimina una venta/compra (`rpc_reverse_stock_movement`, `reference_type='sale_reversal'`/`'purchase_reversal'`, `metadata.reverses_movement_id` apunta al original, que permanece intacto). Las escrituras solo ocurren via RPCs con `SECURITY DEFINER`.

**La edición de una operación también asienta** (`stock-movements-edicion`, 2026-08-19): `rpc_atomic_update_sale_operation` / `rpc_atomic_update_purchase_operation` dejan, por cada línea con producto, el par espejo REVERSE (`type='sale_return'`/`'purchase_return'`, `reference_id`=id viejo, `reference_type='sale_update'`/`'purchase_update'`) + APPLY (`type='sale'`/`'purchase'`, `reference_id`=id **nuevo**, `reference_type='sale'`/`'purchase'` — indistinguible del movimiento de creación). Antes de este change la edición movía `branch_stock` sin dejar rastro: la operación editada quedaba sin movimiento de referencia y su eliminación no reponía stock, en silencio (204 operaciones vivas en prod, backfill `scripts/sql/backfill_stock_movements_operaciones.sql` ejecutado 2026-08-19).

Un movimiento `reference_type IN ('sale','purchase')` que apunte a una fila ya eliminada **con** su contramovimiento de cierre (`*_reversal` para eliminación, `*_update` para edición) es un **estado válido**, no un huérfano defectuoso — es el rastro esperado de toda eliminación o edición normal. El defecto es un movimiento así **sin** contramovimiento de cierre. El invariante de no-orfandad se verifica como gate de comportamiento sobre datos sintéticos (hacia adelante), no como aserción retroactiva sobre el historial de producción — el desvío previo a `stock-movements-edicion` (139 huérfanos históricos sin cierre) queda documentado y, por decisión del PO, sin tocar.
...
**Umbral fraccionario (`ventas-unidades-conversion`, 2026-09-24)**: `branch_stock.min_stock` pasa de `integer` a `numeric(15,4)`, expresado en la **unidad base del producto** ("avisar cuando queden 0,5 kg"); `rpc_set_product_min_stock` es `(uuid, numeric)` (firma única, `DROP` + `CREATE`), `get_dashboard_critical_stock_items` devuelve `min_stock numeric` y el backend tipa `Decimal` con `ge=0` (negativo → 422; `GREATEST(…, 0)` en la RPC como red). Un mínimo entero de un producto por unidades no cambia.
...
**importador-productos-fastapi (2026-09-10)**: la importación masiva de productos por CSV deja de invocar `rpc_bulk_upsert_products` directo desde el navegador — pasa por `POST /products/import` → `rpc_import_products` (`SECURITY DEFINER`), que llama a `rpc_bulk_upsert_products` **una sola vez con el archivo completo** dentro de una subtransacción **todo o nada** (una fila inválida rechaza el lote entero, sin escritura parcial). `rpc_bulk_upsert_products` queda **revocada de `authenticated`**: ya no es invocable directo por PostgREST, sólo desde `rpc_import_products` (que sí exige `is_account_writer`, guard que el upsert nunca tuvo) y desde `service_role`. Su único cambio de comportamiento es que el error de fila ahora informa el número de línea del archivo (`row`, opcional). Tope de 2.500 filas por archivo (bajado de 5.000 en la revisión de código previa al merge — cubre igual el mayor lote real medido, 1.393, y el catálogo más grande, 2.372), idempotencia por clave + dedupe por hash de archivo (tabla `product_imports`). El límite de productos del plan (`plan-gating`) **NO se aplica todavía a la importación masiva** — sólo al alta de a uno — porque OQ-1 de este change no tiene sign-off del PO; es un hueco conocido y documentado, no un olvido.

### RN-24 — Stock fraccionario y conversión de unidades (actualizada — `ventas-unidades-conversion`, 2026-09-24)
`branch_stock.quantity`, `stock_movements.quantity_delta` y las cantidades de línea (`sale_items`, `sales_order_items`, `purchase_items`, `sales`, `purchases`) son `NUMERIC(15,4)`, soportando cantidades como `0.5 kg`, `2.350 litros`, etc. Las unidades de medida (`units_of_measure`) definen el factor de conversión relativo a la unidad base del mismo `type`.

**El stock se lleva siempre en la unidad base del producto** (`products.base_unit_id`). La línea conserva la cantidad y la unidad que ingresó el usuario (450 g); el delta del ledger se expresa en la base (0,45 kg). La conversión tiene **una sola definición**, `_uom_normalize_quantity(producto, unidad, cantidad)` = `cantidad × factor(unidad) ÷ factor(base del producto)`, redondeada a 4 decimales, que consumen los **seis** cuerpos que escriben stock desde una operación: alta de venta (`rpc_create_sale_operation_v2` y la rama legacy del kill-switch `sale_items_rpc_v2` en `rpc_create_sale_operation`), POS (`_c29_confirm_order_core`), edición de venta, alta y edición de compra. Antes del change sólo las dos altas convertían (y "× factor" a secas, relativo a la base del tipo): el POS y las dos ediciones movían la cantidad cruda — 450 g descontaban 450 kg.

Reglas de la definición: (a) sólo entre unidades del mismo `type` — nunca un resultado silencioso (`P0400 unit_type_mismatch`); (b) un producto **sin** unidad base sólo admite unidades base (factor 1, sin `base_unit_id`): Gramo, Mililitro, Docena o Tonelada exigen declarar la base del producto (`P0400 unit_requires_base_unit`) — es lo que convirtió 0,381 "mL" en un movimiento de `-0.0004` el 2026-09-22; (c) una cantidad no nula que se anule al redondear a 4 decimales se rechaza (`P0400 quantity_below_precision`); (d) la reversa de una edición devuelve el `quantity_delta` guardado del movimiento original, nunca la cantidad cruda de la línea (mismo principio que el borrado); (e) **una variante hereda la unidad base de su padre** (`COALESCE(propia, padre)`, en el helper y en `v_products_with_stock.base_unit_id`): el formulario no le asigna una propia a la variante (en prod, 2 variantes tienen una declarada y la conservan: la propia gana) y el padre con variantes sólo se vende a través de ellas; (f) la unidad de la línea tiene que ser del sistema o de la cuenta del producto (`P0404`); (g) **la unidad base efectiva no se cambia debajo del stock** (decisión provisoria D-C, pendiente de sign-off del PO). La hace cumplir **la base de datos para cualquier escritor** — FastAPI, PostgREST, el importador o una función futura — con el trigger `trg_product_base_unit_guard` (`P0409 base_unit_locked`; `P0404 base_unit_not_found` si la unidad no es del sistema ni de la cuenta); `PUT /products` sólo adelanta el mismo `409` con un mensaje tipado. La unidad **efectiva** es la propia o, para una variante que no declara una, la del padre. Cambiarla o quitarla se rechaza si el producto (o las variantes que la heredan) tiene stock ≠ 0 en alguna sucursal o algún `stock_movements` — las cantidades del ledger están en la unidad vigente y cambiarla las reinterpretaría (12 u pasarían a leerse 12 kg). Cuentan como cambio: cambiar o quitar `base_unit_id`, re-parentar o desenganchar una variante que hereda (con bloqueo compartido del padre nuevo, para que un cambio concurrente de su base espere) y borrar físicamente el padre de una variante que hereda y sobrevive (`ON DELETE SET NULL` la desengancharía); borrar el padre junto con sus variantes no se traba. **Asignarla** a un producto que no tenía se permite —es la vía para declarar la base de los productos que se venden por kilo (OQ-1)— salvo que su stock y su historia ya se hayan grabado con **otra unidad explícita**: un producto sin base admite líneas en cualquier unidad base, así que con historia en `u` no se le puede asignar `kg`, y con historia en kg no se le puede asignar `g` (sí `kg`); las líneas sin unidad no traban. En el importador de productos, un re-parent rechazado vuelve como error de fila; (h) **una unidad de medida en uso** (unidad base de un producto o unidad de alguna línea de venta, compra, orden o presupuesto) no cambia de factor, tipo ni base (`P0409 unit_in_use`, trigger `trg_uom_in_use_guard`): el reporting recalcula la cantidad base con el factor vigente (RN-D6) y el stock ya grabado no se re-expresa. Nombre y símbolo se editan libres; para corregir una conversión se crea una unidad nueva. El selector de unidad de POS, venta y compra sólo ofrece lo que esta definición acepta (`lib/unit-utils.ts` → `compatibleUnits`). Gates: `supabase/tests/test_ventas_unidades_conversion.sql` y `supabase/tests/test_ventas_unidades_conversion_race.sh`.

### RN-24-bis — El precio de una línea es por unidad de la línea (`ventas-unidades-conversion`, 2026-09-25 — **provisoria, pendiente del sign-off del PO: es dinero**)
El `amount` / `price` de toda línea de venta, compra, orden o presupuesto está expresado **por unidad de la línea** (la que eligió el usuario), de modo que el importe de la línea es `amount × quantity` en esa unidad — igual que en todas las operaciones históricas (contrato **D-F**). El precio del catálogo está en la unidad base del producto; al vender o comprar en otra unidad, el frontend lo re-expresa con el mismo factor que la cantidad (`convertUnitPrice`): 100 g de un producto a $1.800/kg se cobran $180, y una docena a $100/u, $1.200. **D-F′**: el precio por unidad de la línea **no se redondea** — las seis columnas de precio de línea (`sales.amount`, `sale_items.price`, `sales_order_items.price`, `quote_items.price`, `purchases.amount`, `purchase_items.price`) son `NUMERIC` sin escala ($4.575/kg en gramos es $4,575/g; $1.234,56/kg es $1,23456/g) y el frontend sólo limpia el ruido binario (`roundUnitPrice`, 15 dígitos significativos; también el precio con descuento). Los **importes de dinero** — subtotal y total de la orden, caja, banco, cuenta corriente y el total del evento contable — van **al centavo**, y cuando se derivan de las líneas se redondean **una sola vez** sobre la suma: `round(Σ amount × quantity, 2)`, nunca acumulando sumas ya redondeadas (con dos líneas de 332,667 el cargo es 665,33, igual que la factura). En pantalla, el precio unitario se muestra con su precisión (`formatUnitPrice`: al centavo igual que siempre; sub-centavo con hasta 5 decimales) en el ticket, el PDF y los listados de ventas y compras, y la cantidad con el símbolo de su línea. Residuo declarado: con un precio de más de 2 decimales el POS cobra el subtotal al centavo, así que `amount × quantity` puede diferir del total en menos de medio centavo.
...
**productos-costo-nullable (2026-09-10)**: `products.cost` es un dato **opcional** — la cascada de costo (snapshot de línea, con fallback al catálogo) puede no resolver en ninguno de sus dos peldaños, y todo consumidor SHALL informar esa ausencia como tal, nunca sustituirla por cero (ver capability `product-cost` y RN-D2 de la spec `reporting-invariants`). Por esta misma regla de inmutabilidad, las **9 líneas de venta históricas** con `unit_cost_snapshot = 0` (en 9 productos distintos, medidas al momento del change) **no se reescriben** aunque ese cero provenga del `DEFAULT 0` que la columna tenía antes y no de una declaración real del usuario — es un residuo declarado, no un bug del reporting.
...
Las claves naturales de maestros se protegen con índices únicos **parciales** condicionados a `WHERE deleted_at IS NULL` (además de las condiciones preexistentes): `idx_products_sku_user`, `idx_products_barcode_account_unique` (alcance por cuenta — `UNIQUE(account_id, barcode)`), `cost_centers_account_name_lower_idx`. Un SKU/código/nombre borrado **se puede recrear** (la fila borrada no ocupa la clave); dos maestros *activos* jamás comparten la clave. Solo aplica donde ya existía unicidad — no se inventan claves naturales nuevas (clients/suppliers/bank_accounts/cashboxes no declaran ninguna todavía).
...
### RN-D6 — El reporting cuenta y costea en la unidad base del producto (`ventas-unidades-conversion`, 2026-09-25)
Todo read-model que cuente unidades vendidas o costee una línea de venta expresa la cantidad en la **unidad base del producto** —la misma en que están `unit_cost_snapshot` y `products.cost`—, obtenida de la definición única de conversión (RN-24) a través de su envoltorio de lectura `_uom_quantity_for_reporting`. El importe de la línea no cambia de regla (`COALESCE(total, amount)`, RN-24-bis). Rige para `reporting_sales_lines_in_window` (y por ella ranking, evolución, desgloses, top clientes y rentabilidad), `rpc_dashboard_kpi_summary` y `rpc_dashboard_channel_margin`: 100 g de un producto a $600/kg cuentan 0,1 unidades y $60 de costo, no 100 unidades y $60.000. Una línea histórica que la regla de escritura de hoy rechazaría (la venta en mL del 22-09 sobre un producto sin base) se reporta tal como se grabó en vez de abortar el reporte. La cantidad base no se persiste: se recalcula al leer, y por eso una unidad en uso no cambia de factor (RN-24 (h)). Consumidores pendientes, declarados: `ai-precio` y `fair-advisor` (Edge Functions de IA) todavía leen la cantidad cruda.

### RN-D7 — Alerta de margen bajo por venta (`low_margin_alert`)
Cada venta insertada (trigger `on_sale_insert_margin_check` → `check_low_margin`) calcula el margen de la **línea** como `(importe de la línea − costo por unidad base × cantidad en unidad base) / importe de la línea`, con el importe según RN-D (`COALESCE(total, amount × quantity)`) y la cantidad en unidad base (RN-D6). Si el margen es menor al 15 %, registra un `email_logs` `low_margin_alert` con ese importe y ese costo, y el envío sigue el patrón de RN-40. Un producto sin costo cargado no dispara la alerta. Antes de `ventas-unidades-conversion` comparaba el precio **unitario** contra costo × cantidad cruda: toda venta con cantidad mayor a 1 era un falso positivo (66 de las 71 alertas de prod de los 30 días previos) y una venta en gramos daba márgenes absurdos.
