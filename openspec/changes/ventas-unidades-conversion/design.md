...
2. Migración `20261061000001_ventas_unidades_conversion.sql`, en este orden: helper → cinco `CREATE OR REPLACE` → `ALTER` de `branch_stock.min_stock` (+ guardado de `products.min_stock` para local) → `DROP`/`CREATE` de `rpc_set_product_min_stock` con `GRANT` idéntico al vivo → `REVOKE` del helper → gate embebido de introspección.
...
5. Verificación post-merge en prod: `MAX(version) = 20261061000001`, tipo de `branch_stock.min_stock`, una sola firma de `rpc_set_product_min_stock`, `has_function_privilege` del helper en `false` para `anon`/`authenticated`, los cinco cuerpos vivos con la llamada al helper.
