...
**Estado**: **Scaffolding YA implementado y archivado** (change `fastapi-backend-monorepo`, 2026-06-06): monorepo, FastAPI, auth JWT, WebSocket, tests — de los cuales el **WebSocket se retiró** en 2026-09-16 (ver DEC-16) y la **migración de realtime a WS quedó sin objeto**: el tiempo real es Supabase y nada más. **Pendiente** (CHANGES.md FASE 5): capa de datos (asyncpg + repositories), migración de la API, pagos y desacople del DataContext. Vía Strangler Fig con feature flags.

### DEC-13 — JWT-passthrough transaccional + adopción de rol en el backend (NUNCA service_role)
**Decisión**: El backend Python recibe el JWT del usuario e inyecta los claims en la conexión `asyncpg` **dentro de una transacción explícita por request y con alcance transaccional** (`set_config(..., true)`, equivalente a `SET LOCAL`), y adopta el rol `authenticated` (`SET LOCAL ROLE authenticated`, mismo punto, misma transacción) para que la RLS org-based se **evalúe de verdad** en cada request del backend — no sólo se mantenga escrita como red teórica. No se usa `service_role` en el backend salvo el camino de servicio explícito (webhook de pagos, relay CAE, tarea en segundo plano de CAE, dispatcher del outbox), que retiene `postgres`/BYPASSRLS a propósito por ser operación de máquina sin JWT de usuario.
**Justificación**: Preserva el hardening de seguridad existente (RLS, `WITH CHECK`, triggers anti-escalación, RPCs con `auth.uid()`) y lo hace **efectivo** para el backend, no sólo para el camino navegador→PostgREST. Usar `service_role` en el camino de request obligaría a reimplementar toda la autorización en Python, tirando ese activo.
...
**Estado real vs. aspiracional (corregido 2026-08-26, `v31-tenancy-pool-rls`)**: hasta esa fecha, la implementación real **contradecía** esta decisión en dos puntos simultáneos, ambos ya cerrados — (1) el pool conectaba como `postgres` con `rolbypassrls = true`, con lo que Postgres ni evaluaba las policies (RLS inerte para el backend, causa de H-05); (2) los claims se inyectaban con alcance de **sesión**, sin transacción explícita, lo que bajo Supavisor en *transaction mode* podía dejarlos en una conexión física distinta de la que ejecutaba la query de negocio — causa raíz confirmada del bug K5 (500 intermitente de compras). Ambos puntos se resolvieron en dos pasos independientes, cada uno detrás de su propia palanca de rollout (`TENANCY_TX_SCOPE_ENABLED`, `TENANCY_RLS_ROLE_ENABLED`), **ambas ON en prod** desde el 2026-08-23/24. **Nota de C-17 corregida explícitamente para que no se re-litigue**: "`SET ROLE` no funciona con pgBouncer/Supavisor en transaction mode" es cierto sólo para `SET ROLE`/`set_config` con alcance de **sesión** (sobrevive al `COMMIT` y puede filtrarse al siguiente cliente que recicle la conexión física — un bug de seguridad, no una limitación del pooler). Las variantes con alcance **transaccional** (`SET LOCAL ROLE`, `set_config(..., true)`) se deshacen solas en `COMMIT`/`ROLLBACK` y son exactamente el patrón que PostgREST usa contra el mismo pooler desde siempre. Detalle completo en `knowledge-base/08_arquitectura_propuesta.md` y en `openspec/changes/archive/2026-08-27-v31-tenancy-pool-rls/`.
...
### DEC-16 — Realtime se mantiene en Supabase Realtime (WebSocket Python RETIRADO)
**Decisión**: El tiempo real sigue 100% en Supabase Realtime. El frontend conserva sus suscripciones `supabase.channel(...).on('postgres_changes')`.

> **Actualizado el 2026-09-16 (`auth-hardening-jwt-cookies`, D11).** El WebSocket del backend Python (`core/ws_manager.py`, `routers/ws.py`, `/ws/{room_id}`) ya **no existe**: se **retiró**, no quedó "reservado". La infra que esta decisión dejaba lista para el futuro autenticaba leyendo el token de la **query string** (donde queda en logs, en el `Referer` y en el historial), no verificaba vigencia y nunca tuvo un consumidor en `frontend/` — cero referencias. Mantener una superficie autenticada sin consumidor sólo para conservar la opción era más riesgo que valor; el `ws_manager` se reescribe el día que haga falta. Candado: `backend/tests/test_no_websocket_surface.py`. El resto de esta decisión (por qué Supabase y no WS propio) sigue vigente y es justamente lo que la sostiene.
