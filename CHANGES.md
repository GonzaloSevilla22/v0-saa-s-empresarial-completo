...
**Change `cobranzas-reverso`** 🔨 **APPLY COMPLETO 2026-09-02** (propose + apply en PR **#487**, rama `opsx/cobranzas-reverso-apply` — PR y archive pendientes de merge; `openspec/changes/cobranzas-reverso/`, governance MEDIA con un tramo ALTA — grupos 3, 4, 5 y 6 escriben dinero real en cuatro libros desde RPCs `SECURITY DEFINER` nuevas). Cierra **OQ-4 de `caja-compras-cobranzas`**, firmada por el PO el 2026-09-01 (*"FUERA de este change, por recomendación. Candidato `cobranzas-reverso` dado de alta"*): hoy no existe ningún camino para deshacer un cobro de cuenta corriente ni un pago a proveedor mal cargado.
...
- **Specs sincronizadas** (pendiente de archive): `payment-reversal` (**capability nueva**, 8 requirements) + `customer-account`, `supplier-account`, `cash-movement`, `journal-entry`, `operation-delete-compensation`, `transactional-outbox` (modificadas).
...
- **Leer antes**: `openspec/changes/cobranzas-reverso/design.md` (D1-D13 + las 5 OQs), `openspec/changes/archive/2026-09-02-caja-compras-cobranzas/` (molde literal de la compensación multi-libro que este change extiende al reverso).
