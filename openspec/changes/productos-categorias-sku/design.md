...
### D8 — El gestor de categorías es la décima pestaña de `/configuracion` (sign-off del PO, OQ-2)

**Resuelta por sign-off del PO 2026-09-03, en contra de la recomendación original de este design.** La propuesta era montar el gestor en `/productos` para no llevar la `TabsList` de `/configuracion` de 9 a 10 pestañas. El PO decidió lo contrario y el criterio que da es más fuerte que el de layout: **un solo lugar para los catálogos de la cuenta**. Centros de costo y Formas de pago ya viven ahí; que Categorías de producto viviera en otro lado obligaría al usuario a aprender dos lugares distintos para la misma clase de tarea, y la consistencia de ubicación vale más que ahorrar una columna de grilla.

El gestor va entonces como pestaña "Categorías" en `/configuracion`, con el mismo molde visual que `PaymentMethodManager`, y la `TabsList` pasa de `lg:grid-cols-9` a `lg:grid-cols-10` (`grid-cols-3 sm:grid-cols-5` en los breakpoints menores no cambian: ya envuelven en varias filas).

Lo que la recomendación original quería resolver —que el usuario descubra la limitación mientras carga un producto y no encuentre salida— **no se pierde**: lo cubre el alta rápida inline del selector (D9), que resuelve el caso en el lugar sin mandar a nadie a Configuración, y lo refuerza la recategorización en lote (D14) desde el propio listado de productos. La pestaña es el lugar de la **gestión** del catálogo; el formulario sigue siendo el lugar de la **creación al vuelo**.

Riesgo asumido y declarado: la décima pestaña estrecha una `TabsList` que `qa-integral-modulos` ya tuvo que arreglar en responsive y sobre la que sigue abierto el residuo `tablet-filtros-cta`. Por eso la pasada de verificación visual de este change incluye `/configuracion` en las cuatro combinaciones, y no sólo las pantallas nuevas.
...
### D14 — Recategorización en lote: endpoint FastAPI, no RPC nueva; el padre propaga a sus variantes

**Alcance sumado por el PO el 2026-09-03.** El catálogo por cuenta le da al usuario dónde poner sus productos, pero sin una acción en lote no le da cómo moverlos: son **2.951 productos varados en "Otros"** en una sola categoría, y reclasificarlos de a uno por el formulario no es una opción realista. Sin esta pieza, el change habilita el problema resuelto sólo para los productos futuros.

**Transporte: endpoint FastAPI, no una RPC `SECURITY DEFINER` nueva.** El CRUD de productos ya vive en `/products`, así que la acción en lote es una hermana suya y no un transporte nuevo. La razón de fondo es de seguridad: la operación se expresa como **un solo** `UPDATE … WHERE id = ANY($1) AND account_id = $2`, atómico por definición, donde el `AND account_id` **es** el guard de tenencia — exactamente la regla dura del proyecto (*todo repository filtra explícito por `account_id`, la RLS es red y no guard único*), con la RLS ya efectiva para el backend desde `v31-tenancy-pool-rls`. Crear otra función `SECURITY DEFINER` sumaría superficie privilegiada al gate de ACLs justo en el repositorio que viene de arreglar tres fugas seguidas por funciones privilegiadas (`cuenta-corriente-party-guard`, `tenancy-guard-caja-outbox`, hotfix #454). La opción más segura acá es no crear la función.

**Guards.** La categoría destino se valida contra la cuenta y contra su estado vivo/activo antes del `UPDATE`: una categoría de otro tenant, borrada o inactiva devuelve **404** RFC 7807 con un mensaje que no revela si existe en otra cuenta (mismo criterio que `P0404` de `cuenta-corriente-party-guard`). Los ids de producto ajenos al tenant **no producen error**: simplemente no matchean el `WHERE`, que es la respuesta que no filtra información. La respuesta informa `solicitados` y `actualizados`, de modo que la interfaz pueda decir que algo no se aplicó sin que el servidor confiese por qué.

**Idempotencia.** El `UPDATE` lleva `AND category_id IS DISTINCT FROM $target`: repetir la misma operación deja el mismo estado final, no toca filas de más y no dispara el trigger de espejo al pedo. El contador de actualizados es entonces honesto —cuenta cambios reales, no filas alcanzadas—, con el mismo criterio de `IS DISTINCT FROM` que usa el trigger de renombre (D1).

**Variantes: el padre propaga, y no es una preferencia sino una obligación del invariante.** Este design ya especifica que *la variante hereda la categoría de su producto padre* y que *la variante no puede contradecir al padre* (D11). Si recategorizar un padre no propagara, quedaría un padre en "Indumentaria" con sus variantes en "Ropa": el change violaría su propio invariante en el primer uso. Por lo tanto el servidor **expande cada padre seleccionado a sus variantes** dentro del mismo `UPDATE`.

Simétricamente, un id de variante recibido suelto se **normaliza a su padre** y recategoriza el grupo entero, porque no existe un estado válido donde una variante tenga otra categoría. La interfaz no ofrece seleccionar variantes por separado (sus casillas cuelgan del grupo), así que esa normalización es una defensa para la API, no un camino que el usuario transite. Se declara explícitamente porque es un comportamiento sorprendente si alguien llama la API a mano.

**Selección en la interfaz.** Se reutiliza el patrón que ya existe en el repo: `useState<Set<string>>` + `Checkbox` + helper `toggle`, tal como `ReconciliationBoard.tsx` — y `product-catalog.tsx` ya usa el mismo idioma de `Set<string>` para `expandedIds`, así que no se introduce un patrón nuevo. Sobre esa selección aparece una barra de acción con el conteo y el selector de categoría destino, que **reutiliza `ProductCategorySelect`** (mismo componente, no una lista paralela). La selección se limpia al aplicar y al cambiar la búsqueda, para que nadie recategorice a ciegas un conjunto que ya no está viendo.

**Tope por request: 500 productos.** Una cuenta puede tener 2.951 en "Otros"; el cliente trocea y agrega los resultados. Un tope explícito evita payloads y transacciones sin límite superior conocido, y el troceo es invisible para el usuario. Es un límite de transporte, no de producto.
...
- **[Una recategorización en lote mal apuntada mueve cientos de productos de golpe]** → Es una escritura masiva sobre el maestro y no tiene deshacer propio. Mitigación: la barra de acción declara el conteo exacto y la categoría destino antes de aplicar y pide confirmación; la selección se limpia al cambiar la búsqueda para que nadie aplique sobre un conjunto que ya no ve; y el daño es **reversible con la misma herramienta** —recategorizar de vuelta es otra selección— porque la operación no destruye información: sólo reasigna una FK, sin tocar stock, precios ni documentos. Trade-off aceptado: no se construye un historial de recategorizaciones (sería un ledger nuevo, desproporcionado para un rótulo de maestro).

- **[La décima pestaña estrecha la `TabsList` de `/configuracion`]** → Riesgo asumido por sign-off del PO (D8), a cambio de tener un solo lugar para los catálogos de la cuenta. Mitigación: `/configuracion` entra a la pasada de verificación visual en las cuatro combinaciones, aunque no sea una pantalla nueva de este change.
...
**Las cinco quedaron resueltas por sign-off del PO el 2026-09-03.** Se conservan con su resolución porque el razonamiento sigue siendo la justificación de las decisiones que gobiernan.

- **OQ-1 — ¿Cuál es el tope de categorías nuevas por importación?** → **Resuelta por sign-off del PO 2026-09-03: 50**, el valor propuesto. Por encima de eso lo más probable es una columna mal mapeada y no un catálogo legítimo. Vive en una constante única (D6).
- **OQ-2 — ¿El gestor de categorías va también a `/configuracion`?** → **Resuelta por sign-off del PO 2026-09-03, en contra de la recomendación de este design: SÍ, y sólo ahí** — como décima pestaña, junto a Centros de costo y Formas de pago. El criterio del PO es un solo lugar para los catálogos de la cuenta, y pesa más que ahorrar una columna de grilla. D8 quedó reescrita; el caso que la recomendación original cuidaba lo cubre el alta rápida inline (D9) y la recategorización en lote (D14).
- **OQ-3 — ¿Cuándo se retira `products.category` (TEXT)?** → **Resuelta por sign-off del PO 2026-09-03, por la recomendación: no en este change** (D1). Deuda declarada, con el mismo perfil que las columnas legacy de RN-97: su retiro es un change propio que debe migrar `v_products_with_stock` y todos sus lectores de una sola vez.
- **OQ-4 — ¿Las 7 categorías legacy son el seed correcto?** → **Resuelta por sign-off del PO 2026-09-03, por la recomendación: se siembran las 7 vigentes.** Son las que los 5.084 productos ya usan, y cualquier otra elección obligaría a decidir qué hacer con los que quedan fuera. Es un valor por defecto sin privilegio alguno en el código: cada cuenta las renombra o desactiva.
- **OQ-5 — "Otros" concentra el 58% de los productos.** → **Resuelta por sign-off del PO 2026-09-03: entra en alcance la herramienta, no la reclasificación.** El PO sumó la **recategorización en lote** (D14) a este change, que es exactamente la "ayuda posterior" que esta pregunta anticipaba — pero deja de ser un change aparte. Se mantiene firme la otra mitad de la recomendación: el sistema **no reclasifica nada por su cuenta**. Adivinar a qué categoría real pertenece cada uno de los 2.951 productos es una decisión de negocio de cada tenant, no una migración de datos inferible; lo que el change entrega es la herramienta para que la tome en minutos en vez de en semanas.
