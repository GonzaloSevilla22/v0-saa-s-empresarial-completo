...
class StockRepository(BaseRepository):
    async def get_stock_by_product(self, product_id: str, user_id: str) -> asyncpg.Record | None:
    ...
    async def list_movements(self, product_id: str, user_id: str) -> list[asyncpg.Record]:
        return await self.fetch(
            """
            SELECT sm.* FROM stock_movements sm
            JOIN products p ON p.id = sm.product_id
            WHERE sm.product_id = $1 AND p.user_id = $2
