...
**El change activo es `qa-integral-modulos` (ítem 17, apply completo, PR y archive propios pendientes — independientes de los archives de `seguros-perfil-asesor` e ítem 19 `caja-compras-cobranzas`, ambos ya cerrados).** El gating de fondo sigue siendo `v3-rbac-multirole` (ítem 4, CRÍTICO, bloqueado a sign-off del PO; su dependencia `v31-tenancy-pool-rls` ya está **satisfecha**). Lo que quedó documentado y sin resolver:

Heredado de `caja-compras-cobranzas` (ítem 19, ver `CHANGES.md` para el detalle completo):

- **`cobranzas-reverso`** (OQ-4) — hoy un cobro o un pago de cuenta corriente mal cargado no se puede deshacer por ningún camino; con este change ese hueco ya arrastra también un movimiento de caja que quedaría sin revertir.
- **`cobranzas-catalogo-pagos`** (Non-Goal 3) — unificar los modales de cobro/pago al catálogo `payment_methods` completo, espejo de lo que `pos-catalogo-pagos` ya hizo para el mostrador.
