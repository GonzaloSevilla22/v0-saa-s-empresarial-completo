...
`gastos-forma-pago` (ítem 16) no dejó ningún change propio recomendado como "siguiente" — el gating de fondo sigue siendo `v3-rbac-multirole` (ítem 4, CRÍTICO, bloqueado a sign-off del PO; su dependencia `v31-tenancy-pool-rls` ya está **satisfecha**). Lo que quedó documentado y sin resolver:
...
- **Desborde horizontal de `/gastos` por debajo de ~1372px** — deuda preexistente que el fix de la barra de acciones de este change (commit `f65adf9`) **redujo a la mitad** (antes desbordaba desde 1464px) pero no cerró del todo: a 1280px sobran 92px y el CTA queda recortado. Cerrarlo exige encoger el grupo de filtros, que toca componentes compartidos con otras pantallas.
