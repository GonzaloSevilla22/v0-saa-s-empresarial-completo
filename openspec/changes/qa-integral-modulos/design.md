...
Fuente de verdad: `qa/INFORME.md` (copiado a este change; capturas NO versionadas, en el scratchpad de la sesión de QA). 27 hallazgos confirmados (H1–H27) + 3 defectos de backend detectados por consola/red durante la misma corrida. Trazabilidad de los tres (la evidencia primaria del kit ya no existe — el scratchpad se limpió al cerrar el QA, §5 del informe — así que cada uno se ancla a evidencia re-verificable): el 500 de `GET /sales-orders` está en el informe §4 ("`/ventas/ordenes` devuelve 500 con cualquier pedido cargado") y su causa raíz se re-verificó contra prod (la tabla `sales_orders` NO tiene columna `payment_method` — solo `payment_method_id uuid` — mientras `schemas/sales_orders.py:101` la declara requerida; verificable con un `SELECT` a `information_schema.columns`); el `42804` de `rpc_product_profitability` NO figura en el informe — salió de la escucha de consola del kit de QA — y se re-verificó de forma reproducible (`sales.date` es `timestamptz` en prod y la función declara `last_sale_date date` en el `RETURNS TABLE` con `MAX(s.date)` en el SELECT, `20260814000001_v3_reporting_invariants.sql:132,175`); el `PGRST200` del embed `account_members→profiles` está en el informe §4 dos veces (consola limpia salvo errores conocidos, y "El módulo Equipo hereda el 400 ya conocido"). El QA corrió íntegramente sobre el stack local; producción intacta. El principio rector de este change es **agrupar por causa raíz**: 17 de los 27 síntomas colapsan en dos archivos del design system.
...
Ninguno de los 27 queda sin destino; no hay hallazgos fuera de alcance. Los tres ítems "(consola)" no llevan número H: salieron de la escucha de consola/red de la misma corrida (dos constan en el informe §4; el `42804` solo en el kit — la trazabilidad exacta de cada uno está en §Context), y se cierran acá porque comparten pantalla y causa con lo auditado.
...
**Elegido: variante de (2) que conserva el portal — `PopoverPrimitive.Portal` con `container`.** `DialogContent` (`ui/dialog.tsx`) y `SheetContent` (`ui/sheet.tsx`) publican su nodo de contenido en un contexto React propio (`DialogContainerContext`); `ui/popover.tsx` lo consume y, si existe, pasa ese nodo como `container` del Portal. Resultado: dentro de un modal el popover cuelga del subárbol del contenido → queda dentro del shard de `react-remove-scroll` → el gesto no se cancela (exactamente lo que probó la contraprueba). Fuera de modales el contexto es `null` y el Portal sigue yendo a `document.body`: **cero cambio de comportamiento para los popovers sanos**. **Advertencia CSS que el apply debe asumir como resultado esperado, no como edge case (R1)**: el `DialogContent` actual (`ui/dialog.tsx`) tiene `translate-x-[-50%] translate-y-[-50%]` — un `transform`, que convierte al nodo en containing block de sus descendientes `position:fixed` — Y `max-h-[90dvh] overflow-y-auto` (con lo que `overflow-x` computa `auto`): un popper `fixed` colgado de ese subárbol queda sujeto a su clipping/scroll, así que el desplegable que se extienda por debajo del diálogo **se recorta o genera scroll interno con el CSS de hoy**. Opciones concretas para resolverlo en el apply (en orden de preferencia): (i) quitar el `overflow-y-auto` del `DialogContent` y mover el scroll a un wrapper interno sin `transform`; (ii) posicionar el popper con `strategy: absolute` dentro del container; (iii) promover el plan C — registrar el popper como shard de `react-remove-scroll` vía la prop pública `shards` de `RemoveScroll` (no exige parchear Radix) — a plan A si la verificación de 1.6 confirma el recorte. `SheetContent` no tiene `transform` en reposo ni `overflow`, así que el camino del drawer es seguro tal cual.
...
**RED del grupo** (corre en **navegador real** — Playwright/CDP sobre la infraestructura de la task 0.2, NO en vitest/jsdom: jsdom no implementa layout ni scroll ante `wheel`, así que el assert fallaría antes Y después del fix): (i) test que reproduce el bug — selector abierto dentro de un Dialog, `wheel`/`touch` despachado, `scrollTop > 0` esperado (hoy falla); (ii) test del comportamiento EXISTENTE sano — popover fuera de modal sigue portalizado a `document.body`, cierra por click-afuera y Escape (hoy pasa, protege contra la regresión).
...
**RED del grupo** (corre en **navegador real** — Playwright sobre la infraestructura de la task 0.2, NO en vitest/jsdom: en jsdom `scrollWidth` es siempre 0 y el assert pasaría trivialmente ANTES del fix; y assertear `min-w-0` en el string de `className` es la aserción trivial que el TDD estricto del proyecto prohíbe): (i) test de layout que monta el shell con contenido ancho en un viewport de 390 y afirma `scrollWidth <= innerWidth` (hoy falla); (ii) test del comportamiento EXISTENTE en desktop — el sidebar colapsable y el inset no cambian de dimensiones a 1440 (protege el layout sano).
...
- `GET /sales-orders`: el `SELECT *` de `backend/repositories/sales_order_repository.py:144` (`list_orders`, L140-149) no trae `payment_method` que `SalesOrderOut` (schemas/sales_orders.py:101) exige. Fix mínimo: el repositorio proyecta/deriva la columna que el schema declara (o el schema la vuelve `Optional` si la columna no existe en la tabla — decidir en apply mirando el DDL vivo; el criterio es que el contrato Pydantic y la proyección SQL queden consistentes y el listado devuelva 200 con datos sembrados).
- `rpc_product_profitability`: 42804 (`last_sale_date date` en el `RETURNS TABLE` vs `MAX(s.date)` timestamptz). Elegido: **cast consciente de zona horaria en el SELECT** — `(MAX(s.date) AT TIME ZONE 'America/Argentina/Mendoza')::date`, o el patrón que `public.reporting_local_today()` ya canoniza (el cuerpo vigente de la MISMA función ancla `v_since_date` a ese helper, `20260814000001:159`, por exactamente este motivo). Un `::date` desnudo castearía con la TimeZone de sesión (UTC en Supabase) y reintroduciría el off-by-one que el barrido `20260907000001_business_day_timezone_sweep` (RN-D5) ya eliminó: una venta de 21:00–23:59 ART quedaría con `last_sale_date` del día siguiente. Conserva la firma y el tipo que los consumidores ya esperan (sin `DROP`); cambiar el tipo de la firma rompería al frontend que formatea fecha. Misma migración `20261016000001`, baseline vivo.
...
**Auditoría de daño histórico** (paso estándar del proyecto — precedente de `cuenta-corriente-party-guard` y `tenancy-guard-caja-outbox`): proveedores YA soft-deleteados con `supplier_accounts.balance ≠ 0` quedarían con deuda inalcanzable igual que antes del guard. Medido en prod el 2026-08-31 (`suppliers ⋈ supplier_accounts`): **0 filas** — no hay daño histórico que reparar hoy. Se re-mide en el apply y, si aparece alguno, se decide con el PO (restaurar el proveedor o saldar la cuenta).
...
`rpc_create_expense` ya llama a `c28_register_cash_movement` (con 4 args, omitiendo el 5º parámetro `p_description` — `20261015000001:391`) y a `_pay_register_operation_bank_movement` (con `NULL` literal en la última posición, `p_description` — L413-417). Fix: pasar la descripción del gasto (`expenses.description`, que la RPC recibe como `p_description` — `20261015000001:177`) en ambas llamadas (dos argumentos), reescritura desde el baseline vivo en `20261016000001`. **Backfill recomendado (OQ-3)**: los movimientos afectados existen solo desde el 2026-08-30 (el change `gastos-forma-pago` es de ese día) — un `UPDATE … SET description = e.description FROM expenses e WHERE … AND description IS NULL` idempotente y acotado por `reference_type` cierra el histórico por centavos. Si el PO lo rechaza, se omite el bloque del backfill sin tocar el resto.
...
### D11 — Una sola migración, cuatro funciones, regla de integridad

`20261016000001_qa_integral_fixes.sql` reescribe `rpc_branch_report`, `rpc_product_profitability`, `rpc_create_expense` y **`rpc_delete_expense`**. La cuarta la suma la task 16.2: el motivo del gasto tiene que llegar también a las reversas del borrado, no solo al alta — su baseline vivo (`baseline/rpc_delete_expense.live.sql`, md5 `819bd694…`) se capturó en el apply, con el mismo procedimiento que las otras tres. Checkpoint obligatorio ANTES de escribir SQL: hashear el `pg_get_functiondef` vivo de prod de las cuatro (lección de `compras-proveedor-cuenta-corriente`: el archivo de migración puede haber divergido del cuerpo vivo por reescrituras in-place). `DROP+CREATE` resetea ACLs → re-aplicar los `GRANT`/`REVOKE` vigentes en el mismo archivo (lección advisor 0028). Sin `RAISE` nuevos con ERRCODEs no mapeados; nada de `P0001`.

## Risks / Trade-offs

- **[R1] El portal con `container` dentro del DialogContent SE RECORTA con el CSS actual — es el resultado esperado, no un edge case**: el `transform` del `DialogContent` lo hace containing block del popper `fixed` y su `overflow-y-auto` lo somete a clipping/scroll (detalle y mitigaciones (i)/(ii)/(iii) en D1). La verificación de 1.6 discrimina cuál mitigación hace falta; si (i) y (ii) no alcanzan, se promueve el shard (plan C, vía `shards` público de `RemoveScroll`) a plan A, documentándolo.
...
- **[R3] Cuatro reescrituras de RPC en una migración** (tres previstas + `rpc_delete_expense`, que sumó la task 16.2): si una diverge del cuerpo vivo, el checkpoint la frena antes de escribir SQL (D11) — no se avanza con baseline de archivo.
