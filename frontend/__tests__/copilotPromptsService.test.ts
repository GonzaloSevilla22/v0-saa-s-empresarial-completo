/**
 * fix/admin-timeseries-feria-copilot: mismo bug que PR #482
 * (fix/admin-seguros-timeseries) en `insuranceService.processTimeSeries`,
 * replicado acá. `processTimeSeries` alimentaba el gráfico "Uso de Prompts"
 * de /admin/copilot-ia con `{ name: "Ene".."Dic", value }` —
...
import { copilotPromptsService } from "@/lib/services/copilotPromptsService"

describe("copilotPromptsService.processTimeSeries", () => {
  it("n=0: sin filas produce una serie vacía (no un bucket basura)", () => {
    expect(copilotPromptsService.processTimeSeries([])).toEqual([])
  })

  it("n=1: una sola fila produce un único punto con activations=1 y period parseable como fecha real", () => {
    const result = copilotPromptsService.processTimeSeries([{ created_at: "2026-03-05T12:00:00.000Z" }])
  ...
  it("REGRESSION: el shape es { period, activations } directo — sin name/value/umv_achieved inventados", () => {
    const result = copilotPromptsService.processTimeSeries([{ created_at: "2026-03-01T00:00:00.000Z" }])
  ...
  it("REGRESSION: agrega por AÑO+mes — marzo 2026 y marzo 2027 no se mezclan en el mismo bucket", () => {
    const result = copilotPromptsService.processTimeSeries([
  ...
  it("cada period cae en el primer día del mes (UTC), no en la fecha exacta de la fila", () => {
    const result = copilotPromptsService.processTimeSeries([{ created_at: "2026-06-27T18:45:00.000Z" }])
