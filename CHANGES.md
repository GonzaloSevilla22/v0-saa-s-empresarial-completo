...
- **`delete-guard-ledgers`** ⭐ — **candidato destacado por `pos-banco-movimientos` (su OQ-2)**: eliminar una venta/compra con `bank_movement`/`cash_movement`/`customer_account_movement` posteado hoy es un `DELETE` directo sin RPC ni guard (deja el movimiento huérfano) — el mismo hueco existe para los tres ledgers desde `pagos-cableados-restantes` (#425) y se reconfirmó al agregar el bancario. Cerrarlo bien pide decidir entre bloquear el borrado (mismo criterio `P0423` que ya usa la edición) o emitir un contra-movimiento, y alcanza a los tres ledgers a la vez — no tiene sentido resolverlo ledger por ledger.
- ~~`asiento-venta-formulario`~~ — **archivada 2026-08-20** (ver entrada propia más arriba). Candidatos derivados abiertos: `delete-guard-ledgers` (impedir borrado de operaciones con ledger posteado), superficie del libro diario / OQ-5 (pantalla `/reportes/libro-diario`), y retrofit `posted_at` en las cuatro ramas existentes (hoy postean con la hora del relay; nueva rama ya trae día de la operación).
