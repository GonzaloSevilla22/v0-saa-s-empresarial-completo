...
**Hay dos changes con apply completo y PR/archive pendientes**: `qa-integral-modulos` (ítem 17) y `cobranzas-catalogo-pagos` (ítem 21, PR #489) — independientes de los archives de `seguros-perfil-asesor`, `caja-compras-cobranzas` (ítem 19) y `cobranzas-reverso` (ítem 20), los tres ya cerrados. El gating de fondo sigue siendo `v3-rbac-multirole` (ítem 4, CRÍTICO, bloqueado a sign-off del PO; su dependencia `v31-tenancy-pool-rls` ya está **satisfecha**). Lo que quedó documentado y sin resolver:
