...
- [x] 6.1 Ejecutar los 6 archivos del safety net: 0 regresiones respecto del baseline de 1.1 — **93/93 passing** (71 originales + 22 nuevos en `captcha-freshness.test.ts`/`LoginPage.test.tsx`)
- [x] 6.2 Ejecutar la suite frontend completa y comparar con el total de 1.3 — **1091/1093 passing** (vs. baseline 1053/1071); los 2 fallos son el mismo test preexistente y flaky de `SuscripcionesAmbiguasPage.test.tsx` (timing de `window.location.href`, ajeno a auth/captcha) que ya fallaba en el baseline
- [x] 6.3 `pnpm tsc --noEmit` (o el gate de tipos del proyecto) sin errores y sin un solo `any` introducido — **17 errores preexistentes, ninguno en archivos tocados por este change** (todos en `e2e/*.spec.ts` por falta del paquete `@playwright/test` en este sandbox, y en `__tests__/hooks/use-critical-stock.test.ts`/`__tests__/reporting/*` ajenos a captcha)
- [x] 6.4 Verificar que la suite Playwright E2E de auth pasa sin cambios: con el stub local la fase nunca sale de `ready` (D7) — no se agregó ninguna rama nueva de stub — **no se pudo ejecutar la suite en este sandbox** (`@playwright/test` no está instalado — `ERR_PNPM_RECURSIVE_EXEC_FIRST_FAIL` — y faltan credenciales de Supabase local/QA que exige `env-guard.ts`); verificado en su lugar **por construcción**: `git diff origin/main -- frontend/components/auth/CaptchaWidget.tsx` da vacío (cero cambios, D1/D7 intactos) y el modo stub (`NEXT_PUBLIC_PLAYWRIGHT_LOCAL=true`) nunca registra el listener de visibilidad ni emite `onExpire`/`onError` (líneas 88-99 del widget), así que la fase de la compuerta pasa `cold → ready` una sola vez y no vuelve a moverse — exactamente el comportamiento que ejercitan los tests unitarios del hook (grupo 3)
- [x] 6.5 Verificación visual manual del estado de renovación en las 4 pantallas: desktop + mobile, tema claro + oscuro, usando tokens semánticos del design system (nada hardcodeado) — hecha con `pnpm dev` local + Browser pane; **encontró y corrigió un gap real**: faltaba la clase `aria-disabled:opacity-50` (el estado `aria-disabled` no tenía NINGÚN tratamiento visual — el botón se veía 100% normal). Corregido en las 4 pantallas; confirmado en vivo con `getComputedStyle` (opacity 1→0.5 al simular `aria-disabled`, `pointer-events` se mantiene `auto` — sigue siendo clickeable) en las 4 pantallas, en tema claro y oscuro (Login), y a 375px de viewport (mobile, sin overflow horizontal)
