# business-day-timezone Specification

## Purpose

El día calendario de negocio de toda la app — fecha de alta de operaciones, ventanas de reporting/IA y filtros de "hoy" — se computa en `America/Argentina/Mendoza`, con independencia del huso del servidor y del dispositivo del usuario. Antes convivían tres anclajes (día UTC de las Edge Functions, día local del browser en `lib/date-range.ts` y día argentino real solo en 3 RPCs de reporting), lo que corría un día toda operación cargada entre las 21:00 y las 24:00 hora argentina. Cubre el path de escritura (formularios de venta/compra/gasto), el path de lectura (dashboard, IA, gráficos) y el sweep SQL de las funciones de reporting vigentes. Las fechas de comprobantes fiscales AFIP/WSFE quedan fuera de este canon.
