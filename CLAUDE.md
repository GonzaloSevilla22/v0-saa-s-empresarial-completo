...
17. **`qa-integral-modulos`** 🔨 **PROPUESTA 2026-08-31 · APPLY COMPLETO 2026-09-01** (artefactos en `openspec/changes/qa-integral-modulos/`; los 16 grupos implementados en la rama `opsx/qa-integral-modulos-apply` con re-QA visual y revisión adversarial pasadas — **PR y archive pendientes**; governance LOW/MEDIUM por grupo — MEDIUM lo que toca `ui/popover.tsx`, `ui/sidebar.tsx`, el sistema de toast, el guard de proveedor, billing y las 3 reescrituras de RPC). Cierra por **causa raíz** los 27 hallazgos confirmados del QA integral del 2026-08-30 (informe copiado al change: `qa/INFORME.md` — fuente de verdad) + 3 defectos de backend de la misma corrida. **Dos componentes compartidos explican 17 de los 27 síntomas**: el popover portalizado fuera del shard de scroll del modal (bug del PO: ningún desplegable scrollea dentro de un modal) y el `<main>` del shell sin `min-w-0` (12 pantallas desbordan en móvil escondiendo botones primarios). 16 grupos G1-G16, capability nueva `responsive-shell` + 6 modificadas, migración `20261016000001` (**4** RPCs bajo regla de integridad de función — `rpc_delete_expense` se sumó en el apply), 3 OQs no bloqueantes con recomendación. **Absorbe el candidato del desborde de `/gastos`** anotado abajo. El apply deja **un residuo con nombre propio**: `tablet-filtros-cta` (a 1024px con el riel expandido el CTA de `/ventas`, `/gastos`, `/compras` y `/clientes` queda fuera del viewport inicial porque la barra de *filtros* no wrappea; el documento ya no se estira y el CTA es alcanzable con scroll propio — la pasada de tablet 768–1024 era Non-Goal declarado, así que la cláusula del delta se acotó a móvil en vez de prometer lo que no cumple). Detalle en `CHANGES.md`.
...
**El único change con apply completo y PR/archive pendientes es `qa-integral-modulos` (ítem 17)** — independiente de los archives de `seguros-perfil-asesor`, `caja-compras-cobranzas` (ítem 19), `cobranzas-reverso` (ítem 20) y `cobranzas-catalogo-pagos` (ítem 21), los cuatro ya cerrados; con este último la saga de cobranzas queda **completa**. El gating de fondo sigue siendo `v3-rbac-multirole` (ítem 4, CRÍTICO, bloqueado a sign-off del PO; su dependencia `v31-tenancy-pool-rls` ya está **satisfecha**). Lo que quedó documentado y sin resolver:
