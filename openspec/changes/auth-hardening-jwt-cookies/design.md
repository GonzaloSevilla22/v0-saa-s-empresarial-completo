...
Se reemplazan por un manejador de ruta `GET /api/auth/status`, que lee la cookie httpOnly, **fuerza** una renovación contra el proveedor (que es la única forma de observar un `email_confirmed_at` recién cambiado) y devuelve `{ email, email_confirmed_at }` — nunca tokens. La pantalla conserva su sondeo de 4 s contra ese endpoint y su contador de 30 s para el reenvío, que pasa por una Server Action.
