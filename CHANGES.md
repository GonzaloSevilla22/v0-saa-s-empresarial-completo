...
- `compras-proveedor-cuenta-corriente` — 🔨 **propuesta 2026-08-22** (artefactos en `openspec/changes/compras-proveedor-cuenta-corriente/`: capability nueva `supplier-directory` + 5 modificadas, 96 tasks TDD-first, 6 OQs con recomendación; apply pendiente, governance MEDIUM). Alcance original: ABM de proveedores + selector en el form de compra + cargo real en `supplier_accounts` vía el helper `_pay_register_party_charge` (ya listo del lado `supplier`, ejercitado por tests). Hoy inalcanzable: 0 proveedores en prod, sin selector en el form, sin ABM.
