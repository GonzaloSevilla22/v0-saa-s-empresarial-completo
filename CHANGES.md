...
- ~~**h1 — Caja de otro tenant escribible desde el POS**~~ → **tiene change propio: `tenancy-guard-caja-outbox`** (propuesto 2026-08-23, ver entrada propia abajo). Severidad **ALTA**, **reproducido en local**. `_c29_confirm_order_core` acepta un `p_cash_session_id` ajeno y lo pasa a `c28_register_cash_movement`, que no valida tenencia → ingreso fantasma en el arqueo de la víctima. Alcanzable también por `rpc_quick_sale` y `rpc_confirm_sales_order`. Nota: `_c29_confirm_order_core` es justamente la única entrada de la allowlist del chequeo (4) del gate de ACLs.
- ~~**h2 — Outbox sin filtro de tenant**~~ → **tiene change propio: `tenancy-guard-caja-outbox`** (propuesto 2026-08-23, ver entrada propia abajo). Severidad **ALTA**, **reproducido**. `rpc_process_outbox_batch(int)` y `rpc_mark_event_processed(uuid)` (`SECURITY DEFINER`, `GRANT` a `authenticated`, `20260718000001` L172/L203) devuelven y marcan eventos de **todos** los tenants. Agravado por `POST /outbox/process-pending` (`backend/routers/outbox.py` L32) **sin gating de admin**. El chequeo (4) del gate no las atrapa (son `rpc_*`, fuera del filtro de nombre por diseño).
...
**Change `tenancy-guard-caja-outbox`** 🔨 **PROPUESTO 2026-08-23** (propose PR **#459**; artefactos en `openspec/changes/tenancy-guard-caja-outbox/`). Recoge los hallazgos laterales **h1** y **h2** que dejó anotados `cuenta-corriente-party-guard` y les da tratamiento propio. **Governance: CRÍTICO** (seguridad multi-tenant + dinero + contabilidad) → **el apply NO arranca sin sign-off explícito del PO**: el gate está escrito como task 0.1 🛑 al inicio de `tasks.md` y repetido en la cabecera del `proposal.md`. El PO autorizó *proponer*, no *aplicar*. **Sin superficie frontend** (declarado y verificado: ninguna pantalla llama a `POST /outbox/process-pending`; el único efecto en UI es un `P0422` que la superficie no puede provocar).

- **h1 — el POS escribe en la caja de otro tenant.** `_c29_confirm_order_core` (`SECURITY DEFINER`, `authenticated`, md5 vivo `cecd8c5454611f267a5e131d73bf7928`, len 15110) valida la orden, la forma de pago, la sucursal y el banco, pero del `p_cash_session_id` **sólo chequea `IS NULL`** y lo pasa crudo a `c28_register_cash_movement` — que es **`SECURITY INVOKER`** (md5 `510adc8e150fb5c315e6e9a2635eaff8`) y sólo exige `status='open'` + sucursal activa, sin mirar `account_id`. Como sus cinco llamadores son DEFINER y corren como `postgres`, la RLS no interviene. Reproducido en local: ingreso fantasma en el arqueo de la víctima. El **formulario** de venta sí lo cierra (`cash_optin_requires_open_session` → `P0422`): el guard existía y no se replicó en el POS. Exposición medida en prod: **los 65 movimientos de caja vivos pasaron por ese único camino** (los 65 con `reference_id` en `sales_orders`), y hay **3 sesiones abiertas de 3 tenants distintos**.
- **h2 — el outbox no filtra por tenant.** `rpc_process_outbox_batch(int)` y `rpc_mark_event_processed(uuid)` (`SECURITY DEFINER` + `GRANT` a `authenticated`, `20260718000001` L172/L203, nunca revocadas) devuelven y cierran eventos de **todos** los tenants. Reproducido: un `authenticated` de A lee el payload completo de eventos de B y los marca procesados → el dispatcher real nunca postea ese asiento (fuga de datos **y** denegación de servicio contable). Agravado por `POST /outbox/process-pending` sin gating de admin, con el pool corriendo como owner. El contraste correcto está al lado: `rpc_process_outbox_dispatch` ya tiene `authenticated = false`.
- **h2 bis — hallazgo NUEVO del propose, verificado: hay dos relays compitiendo por el mismo flag.** `rpc_process_outbox_dispatch` (pg_cron, cada minuto) corre **4** consumers —AuditLog, Email, **JournalEntry**, **Notification**— y recién ahí marca `processed_at`. `OutboxRelayService` (endpoint Python) corre **2** y marca `processed_at` igual. Los dos leen `WHERE processed_at IS NULL`: **todo evento que gane el relay Python pierde para siempre su asiento y su notificación**. Tres confirmaciones independientes de que el que sobra es el Python: (i) el spec vigente de `transactional-outbox` ya exige los 4 consumers en orden — el relay Python **incumple el spec como está escrito hoy**; (ii) el pg_cron llama a la RPC, no al endpoint, y su propio comentario lo llama "trigger manual/secundario" (el docstring del router, que dice lo contrario, está desactualizado desde el pivot de C-25); (iii) `v31-tenancy-pool-rls` ya lista `events` y `email_logs` entre las colisiones del Paso 2, o sea que ese camino se rompe solo el día que se encienda la palanca.
- **El change**: guard de **sucursal** en `_c29_confirm_order_core` (copiando literal el predicado del formulario, `P0422`) **+** backstop de **tenant** en `c28_register_cash_movement` (`P0401`, resolución por `cash_sessions → cashboxes → branches.account_id`, **sin cambiar la firma** — la misma resolución que `rpc_register_cash_movement` ya hace) **+** el endpoint del outbox pasa a `get_service_conn` + `require_platform_admin` + `rpc_process_outbox_dispatch`, se retira `OutboxRelayService`, y recién entonces se revocan las dos RPCs del outbox **+** chequeo **(5)** permanente en `test_function_acl_gate.sql` (lista curada de funciones que **leen o actualizan** `public.events`) **+** auditoría read-only del daño histórico. Migración `20261012000001` (MAX vivo verificado = `20261011000001`, 260 migraciones; **el número se re-verifica en el apply** — en el change anterior se movió tres veces).
- **La decisión difícil, y cómo se resolvió**: un `REVOKE ... FROM authenticated` a secas rompería el endpoint el día que se encienda el **Paso 2** de `v31-tenancy-pool-rls` (PR #453, palanca `TENANCY_RLS_ROLE_ENABLED`, hoy OFF), pero **filtrar por tenant tampoco sirve**: el spec exige que el relay lea cross-account, filtrarlo lo vuelve inútil y encima no arregla la supresión de asientos. La salida es mover el endpoint al **camino de servicio**, que por contrato de `v31-tenancy-pool-rls` D5 **nunca adopta el rol `authenticated`** — con eso el revoke sobrevive al Paso 2. Ese contrato hoy vive sólo en un docstring, así que el change lo ancla con un test (OQ-6).
- **Alcance conjunto, grupos separados**: h1 y h2 van en el mismo change porque son la misma clase de bug, comparten los dos archivos de CI (`KPI_Validation.yml` y `test_function_acl_gate.sql`) y necesitan un solo sign-off CRÍTICO — pero **no comparten un solo objeto de base de datos**, así que `tasks.md` los mantiene en grupos independientes (3 = h1, 4-5 = h2) y la migración los separa en dos secciones.
- **Daño histórico preliminar: 0 en los tres conteos** (movimientos de caja cross-tenant por orden: 0; por venta: 0; eventos procesados sin asiento: 0 sobre 464 elegibles y 465 asientos). Se re-mide en el apply; > 0 es checkpoint 🛑 con script fuera de `supabase/migrations/` (RN-99: un movimiento fantasma en una sesión cerrada con arqueo firmado se corrige con contra-movimiento, no con `DELETE`).
- **8 OQs con recomendación.** **OQ-0 🛑** = el sign-off en sí. **OQ-1 🛑** = ¿sale h2 como hotfix inmediato al estilo #446/#454? (*recomendación: sí h2, no h1* — h2 es un PR chico con rollback de dos líneas; h1 exige reescribir una RPC de 15 110 caracteres desde el baseline vivo, que no se hace con urgencia de hotfix). Las demás: OQ-2 las dos capas, OQ-3 retirar el relay Python acá, OQ-4 no revocar `c28_register_cash_movement` todavía, OQ-5 decidir la reparación con los datos a la vista, OQ-6 anclar el contrato de `get_service_conn` con un test, OQ-7 el chequeo (5) no cubre `INSERT`.
- **Hallazgo de diseño que sólo aparece si se mira**: el backstop de tenant **degrada** el gate de comportamiento embebido en `20260804000003` (invoca el helper sobre un anchor cuyo usuario no queda en `account_members` de la cuenta del anchor). **No aborta `db reset` siempre que el ERRCODE no sea `P0001`** — el bloque tiene `EXCEPTION WHEN raise_exception`, que matchea sólo ese código. Con `P0401` degrada a `NOTICE`. La cobertura perdida (saldo por `SUM(amount)` y no `MAX(balance_after)`) se **replica** en el gate nuevo con un tenant bien provisionado; `20260804000003` **no se edita** (es una migración ya aplicada en prod, editarla es el anti-patrón de la regresión de julio).
- **Specs delta**: `sales-order` (+1 ADDED), `cash-movement` (+1 ADDED), `transactional-outbox` (+2 ADDED, 1 MODIFIED), `account-tenancy` (+1 ADDED). `openspec validate --changes --strict` → 6/6.
- **Próximo**: 🛑 **sign-off del PO** → `/opsx:apply tenancy-guard-caja-outbox`.
