...
- **Candidatos que deja**: migrar las dos copias JS de "top productos" (`supabase/functions/ai-insights`, `frontend/lib/ai/buildBusinessSnapshot.ts`) a `rpc_product_ranking`; migrar los 3 reportes existentes a `components/charts/`; **E2** (`rpc_sales_breakdown` canal/sucursal/día/hora + `rpc_sales_top_clients`, sobre el mismo helper y `reporting_plan_window`) y **E3** (export CSV, `rpc_product_sales_evolution` + `/estadisticas/productos/[id]`, `ai-estadisticas`); 12.6 (verificar en prod `MAX(version) = 20261024000001`, ACLs y el conteo 218 → 0 de `last_sale_date` corridos) y 14.3 (demo real del PO) tras el merge.
- **Leer antes**: `openspec/changes/estadisticas-ventas/design.md` (D1-D15, OQs firmadas y notas de implementación de E1), la cabecera de `supabase/migrations/20261024000001_estadisticas_ventas_e1.sql` (checkpoints vivos + desvíos declarados).
