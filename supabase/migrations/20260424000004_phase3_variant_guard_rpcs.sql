-- Phase 3: variant guard — rpc_atomic_create_sale
-- Rule: a product with is_variant = false that has variant children
--       is a parent catalogue entry and cannot be sold directly.
--       Split into a separate file from the purchase RPC because
--       supabase db push executes each file as a single prepared statement
--       and PostgreSQL rejects multiple PL/pgSQL function bodies in one statement.
