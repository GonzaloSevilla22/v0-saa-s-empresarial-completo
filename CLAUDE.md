...
11. **Sin change propuesto — la decisión es del PO.** No queda ningún change con propose hecho esperando apply. Los candidatos reales están en `CHANGES.md` §"Changes candidatos" (incl. `operacion-party-guard`, FK compuesto, `historial-compras-por-proveedor`, `tarjeta-settlement-t-n`, `gastos-forma-pago`, retrofit `posted_at`, pantalla de libro diario, percepciones de V2.5) y `v3-rbac-multirole` sigue **CRÍTICO** esperando sign-off. **Recomendación mía, sin sign-off del PO**: lo próximo debería ser un change de seguridad que cubra juntos **h1 + h2** —los dos hallazgos laterales de severidad **alta** y **reproducidos** que dejó `cuenta-corriente-party-guard`—, porque son el mismo tipo de agujero que este change acaba de cerrar y están abiertos hoy: **h1**, el POS acepta un `p_cash_session_id` de otro tenant y le escribe en la caja (`_c29_confirm_order_core` → `c28_register_cash_movement` sin validar tenencia; alcanzable también por `rpc_quick_sale`/`rpc_confirm_sales_order`); **h2**, `rpc_process_outbox_batch`/`rpc_mark_event_processed` devuelven y marcan eventos de todos los tenants a cualquier `authenticated`, agravado por `POST /outbox/process-pending` sin `require_admin`. Van juntos porque comparten el patrón de fix (guard en el choke point + `REVOKE`) y el mismo gate de ACLs. **No existe todavía ningún artefacto OPSX para esto** — hay que correr `/opsx:propose` primero, y el PO puede perfectamente priorizar otra cosa.
