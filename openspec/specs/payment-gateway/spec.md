...
- **THEN** `accounts.plan_expires_at` se setea a la fecha de vencimiento del período actual, se inserta en `billing_events` con `event_type='cancellation_requested'`, y el plan actual sigue activo hasta esa fecha
