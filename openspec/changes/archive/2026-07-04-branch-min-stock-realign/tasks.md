...
- [x] 4.2 Aplicar la migración en local/preview y correr los gates DO-block (1.3, 1.6, 1.8, 1.10) — todos deben pasar. Aplicada vía CI (`validate-kpis` en PR #265, luego deploy automático a prod al mergear); los 4 gates (más 1.4, 1.11) pasaron en CI (`validate-kpis` verde, run 28712441242) y la migración `20260809000001` quedó registrada en prod (`list_migrations`).
- [x] 4.3 Correr Supabase advisors (security + performance) tras recrear la vista y el RPC; resolver hallazgos nuevos atribuibles a este change. Único hallazgo de `rpc_set_product_min_stock`: advisory estándar "SECURITY DEFINER ejecutable por authenticated" — mismo patrón preexistente que `rpc_adjust_branch_stock` (verificado); no es una regresión, es el patrón aceptado del proyecto (writer-gated). 0 hallazgos de performance atribuibles.
- [x] 4.4 Verificación manual del flujo: editar "Stock Mínimo" en el formulario → confirmar que `branch_stock.min_stock` de todas las filas del producto cambió, que la vista lo refleja, y que una venta que baje la cantidad bajo el nuevo umbral dispara la alerta. Verificado con smoke transaccional (rollback) en prod: producto con 2 branch_stock en 0 → edición a min_stock=8 → ambas filas quedan en 8 → vista `v_products_with_stock` refleja 8 → venta que baja quantity de branch_a a 8 (=min_stock) cumple la condición del trigger (`NEW.quantity < OLD.quantity AND NEW.min_stock > 0 AND NEW.quantity <= NEW.min_stock`). Rollback confirmado (0 filas residuales).
- [x] 4.5 `openspec validate branch-min-stock-realign --strict` verde antes de abrir PR.
