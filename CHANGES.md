...
- OQ-A — `bank_movements` para `transfer`/`card` en el POS: requiere UX de "¿a qué cuenta bancaria entró?"; candidato natural es un change junto a `bank-reconciliation` (ya completa). **Sigue abierta** — es la única de las 5 OQ de `pos-catalogo-pagos` sin change asignado tras `pagos-cableados-restantes`.
...
**Changes candidatos dados de alta por `pagos-cableados-restantes` (design.md Open Questions, sin change propio todavía):**
