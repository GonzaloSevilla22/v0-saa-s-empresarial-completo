...
- **Candidatos que deja (sin resolver acá)**: refactor de `CustomerAccountBalance` a tokens semánticos (D14 — sigue con `text-yellow-400`/`text-emerald-400` literales en su propia pantalla); **Etapa B** = `cobranzas-vencimientos` (ya propuesta: plazos por cliente, `due_date`, aging FIFO, avisos y pestaña "Por pagar"); cablear `test_cobranzas_reverso.sql` a CI.
- **Pendiente para cerrar el change**: merge del PR del apply → verificación post-merge en prod (grupo 8: `MAX(version)` = `20261021000001`, ACLs vivas, panel vs. baseline, re-medición D5) → humo real del PO (cobrar desde la fila y desde la cuenta, ver el panel refrescarse solo) → sign-off OQ-1..4 registrado → archive.
- **Leer antes**: `openspec/changes/cobranzas-panel/design.md` (D1-D14 + 4 OQs), `openspec/changes/cobranzas-vencimientos/` (Etapa B, no tocar hasta cerrar ésta).

---

**Change `cobranzas-vencimientos`** 🔨 **APPLY COMPLETO 2026-09-03** (propose PR **#496** mergeado · apply en rama `opsx/cobranzas-vencimientos-apply`, PR abierto — **pendiente: merge + verificación post-merge en prod + humo real del PO + archive**; `openspec/changes/cobranzas-vencimientos/`, **governance MEDIA con tramo de severidad ALTA**: reescribe las funciones que escriben dinero en cuentas corrientes). **Etapa B del módulo de cobranzas** (sign-off del PO 2026-09-02, las 6 OQs firmadas): crea el dato que a la Etapa A le faltaba — plazo de pago en cascada (`accounts.default_payment_terms_days` → `clients`/`suppliers.payment_terms_days`, todos `NULL` = "sin plazo", NUNCA 0 — D2 opt-in), vencimiento congelado en la fila de cargo del ledger (`customer/supplier_account_movements.due_date`, patrón snapshot D1), imputación FIFO **derivada pura** por línea de flotación (D4, invariante `SUM(abierto) = balance` con gate; el reverso reabre solo, jamás es cargo), aging por 5 tramos (sin-vencimiento propio, D5), `rpc_receivables_report` extendido + `rpc_payables_report` espejo, barrido diario `pg_cron` con dedup por día argentino en 2 capas (D8, 9º/10º tipo del Consumer 4 — el conjunto canónico de 11 del Consumer 3 NO cambia), plantillas de email del digest, pestaña **Por pagar** en `/cobranzas` (OQ-6), recordatorio de WhatsApp con `buildWhatsAppUrl` reutilizado (D12), vencimiento editable en el formulario de venta (el POS usa la cascada sin UI, D11), campo de plazo en client/supplier-form (tri-estado D14) y 9ª pestaña Cobranzas en `/configuracion` (D10).
...
- **Pendiente para cerrar**: merge del PR → verificación post-merge en prod (grupo 11: `MAX(version)`, una definición por función, ACLs, cron job, conteos en 0) → demo real del PO (11.6) → archive. **La pasada visual 4-combinaciones (10.4) quedó pendiente** — sin credenciales locales en la sesión del apply; los asserts de accesibilidad/tokens están en los tests y el sistema de tokens ya garantiza AA (gate `token-contrast-aa`), la verificación visual queda para la demo del PO.
- **Leer antes**: `openspec/changes/cobranzas-vencimientos/design.md` (D1-D15 + 6 OQs firmadas), la cabecera de `supabase/migrations/20261022000001_cobranzas_vencimientos.sql` (hashes + desvíos).
