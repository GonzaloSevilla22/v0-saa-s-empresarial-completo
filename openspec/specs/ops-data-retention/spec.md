...
Retención y reclamo de espacio de los artefactos internos de mantenimiento de la plataforma: logs de ejecución de pg_cron (`cron.job_run_details`), respuestas de pg_net (`net._http_response`) y el log de emails transaccionales (`public.email_logs`). Cubre la función canónica de purga, las ventanas de retención (justificadas por uso medido, no por intuición), la superficie de autorización de esa función, el registro idempotente de los jobs programados y la purga inicial de despliegue. No cubre `analytics_events`, `insights` (datos de producto, no logs de mantenimiento) ni `operation_idempotency` (ciclo de vida ligado al contrato de los RPCs de escritura).
