...
Reglas de la definición: (a) sólo entre unidades del mismo `type` — nunca un resultado silencioso (`P0400 unit_type_mismatch`); (b) un producto **sin** unidad base sólo admite unidades base (factor 1, sin `base_unit_id`): Gramo, Mililitro, Docena o Tonelada exigen declarar la base del producto (`P0400 unit_requires_base_unit`) — es lo que convirtió 0,381 "mL" en un movimiento de `-0.0004` el 2026-09-22; (c) una cantidad no nula que se anule al redondear a 4 decimales se rechaza (`P0400 quantity_below_precision`); (d) la reversa de una edición devuelve el `quantity_delta` guardado del movimiento original, nunca la cantidad cruda de la línea (mismo principio que el borrado); (e) **una variante hereda la unidad base de su padre** (`COALESCE(propia, padre)`, en el helper y en `v_products_with_stock.base_unit_id`): el formulario no le asigna una propia a la variante (en prod, 2 variantes tienen una declarada y la conservan: la propia gana) y el padre con variantes sólo se vende a través de ellas; (f) la unidad de la línea tiene que ser del sistema o de la cuenta del producto (`P0404`); (g) **la unidad base no se cambia debajo del stock** (decisión provisoria D-C, pendiente de sign-off del PO): `PUT /products` permite asignarla a un producto que no tenía y cambiarla o quitarla sólo si ni el producto ni sus variantes tienen stock ≠ 0 en alguna sucursal ni `stock_movements`; si no, `409 base_unit_locked` — las cantidades del ledger están en la unidad vigente y cambiarla las reinterpretaría (12 u pasarían a leerse 12 kg). El selector de unidad de POS, venta y compra sólo ofrece lo que esta definición acepta (`lib/unit-utils.ts` → `compatibleUnits`). Gate: `supabase/tests/test_ventas_unidades_conversion.sql`.
