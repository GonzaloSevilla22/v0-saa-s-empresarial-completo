...
- **Hallazgo real durante la verificación (no cosmético)**: `test_cuenta_corriente_party_guard.sql` (change anterior) resolvía la firma vieja de 6 args de `rpc_register_payment_received`/`_made` por `pg_get_functiondef(...::regprocedure)` en su "candado de cuerpo" — con la firma nueva de 7 args, la resolución fallaba con `function does not exist` antes de llegar a ningún assert. Corregido el literal a la firma nueva (commit `8264d85`); el orden relativo que el candado protege (`bank_account_not_found` < `client_not_found`/`supplier_not_found` < `INSERT` de idempotencia) sigue intacto porque el bloque nuevo del opt-in se insertó ANTES del guard de tenencia, sin mover ninguna de las tres anclas — re-corrido, PASS.
- **Resultado del apply**: backend 1713/1716 (+32 tests nuevos, 0 regresiones, coverage 91,33%), frontend 1782/1785 en 226/227 archivos (+56 tests nuevos; las 3 fallas son TODAS de `AdminSegurosPage.test.tsx`, ajeno a este change — cero relación de import, re-corrido en aislamiento 3 veces dio 9/9, 5/9 y 8/9: flakiness por carga de sistema de esta sesión, no una regresión), `tsc --noEmit` con los mismos 54 errores preexistentes de siempre (ninguno en archivo tocado), 37/39 gates SQL en verde (los 2 restantes: uno es una limitación del harness de esta sesión sin relación con el change — `test_cuentas_billetera_tipo.sql`, que usa `\i` con ruta relativa y necesita correr desde la raíz vía psql local, no vía `docker exec`; el otro fue el hallazgo real de arriba, ya corregido y re-verificado en PASS). **Verificación visual con capturas reales (task 14.1-14.4) bloqueada**: las cuatro pantallas tocadas viven detrás de `/auth/login`, que exige un captcha Cloudflare Turnstile real en este entorno — completarlo está prohibido por las reglas de seguridad del agente, y la sesión no tenía credenciales de un usuario de prueba. Compensado con: (a) los tres bloques de opt-in nuevos son el MISMO markup/clases que el de `expense-form-v2.tsx`, ya auditado visualmente en `gastos-forma-pago` (16 capturas, contraste ≥4,83:1); (b) 26 tests de component/RTL nuevos que fijan cada estado (checkbox marcado/desmarcado/ausente, motivo visible por cada condición); (c) `token-contrast-aa.test.ts` 35/35 sin necesitar entradas nuevas (tonos semánticos reutilizados, cero color literal nuevo). Queda como parte del humo real con el PO (task 15.6, ya obligatoria).
- **Candidatos dados de alta**: `cobranzas-reverso` (OQ-4 — hoy un cobro/pago mal cargado no se puede deshacer por ningún camino) y `cobranzas-catalogo-pagos` (Non-Goal 3 — unificar los modales de cobro/pago al catálogo `payment_methods`, espejo de `pos-catalogo-pagos`).
