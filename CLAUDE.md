...
**El change activo es `qa-integral-modulos` (ítem 17, apply completo, PR y archive propios pendientes — independientes de los archives de `seguros-perfil-asesor`, ítem 19 `caja-compras-cobranzas` e ítem 20 `cobranzas-reverso`, los tres ya cerrados).** El gating de fondo sigue siendo `v3-rbac-multirole` (ítem 4, CRÍTICO, bloqueado a sign-off del PO; su dependencia `v31-tenancy-pool-rls` ya está **satisfecha**). Lo que quedó documentado y sin resolver:

Heredado de `caja-compras-cobranzas` (ítem 19) y `cobranzas-reverso` (ítem 20, ver `CHANGES.md` para el detalle completo):

- **`cobranzas-catalogo-pagos`** (Non-Goal 3 de `caja-compras-cobranzas`) — unificar los modales de cobro/pago al catálogo `payment_methods` completo, espejo de lo que `pos-catalogo-pagos` ya hizo para el mostrador.
