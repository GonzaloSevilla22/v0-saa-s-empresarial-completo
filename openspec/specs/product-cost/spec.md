# product-cost Specification

## Purpose
El costo del producto (`products.cost`) como dato **opcional**: `NULL` significa que no se cargó, `0` significa un costo cero declarado, y los dos son hechos distintos del negocio que el sistema NOT SHALL representar con la misma fila. Define la semántica de la columna en la base, su captura en los tres caminos de entrada (formulario, API, importador), su propagación al snapshot de costo congelado en la línea de venta/compra/presupuesto, y la regla transversal que alcanza a todo consumidor de margen: ningún read-model, pantalla, exportación ni contexto de IA puede sustituir un costo ausente por cero al informar margen, rentabilidad o alertas.
