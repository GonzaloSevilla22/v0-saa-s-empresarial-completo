...
**Change siguiente recomendado: `pos-catalogo-pagos`** — consume las 4 Open Questions gateadas por el PO el 2026-08-19: OQ-1 unificar el POS al catálogo (`rpc_quick_sale`/`rpc_confirm_sales_order` reciben `payment_method_id`), OQ-2 cargo automático en `customer_account_movements` al elegir "Cuenta corriente" (el backend ya lo hace desde C-30 pero ninguna pantalla lo ofrece — ledger en 0 filas), OQ-3 caja obligatoria + `cash_movement` al elegir "Efectivo" fuera del POS, OQ-4 ampliar el CHECK de `sales_orders.payment_method` a `wallet`/`check`. Governance: **MEDIUM/ALTO** — toca `rpc_quick_sale`/`rpc_confirm_sales_order` (caja + cuenta corriente + AFIP), analizar con cuidado antes de escribir código.
