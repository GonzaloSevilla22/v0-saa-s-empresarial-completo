...
La única fuente de telemetría de producto: qué eventos emite el sistema cuando ocurre una operación de negocio (venta, compra, gasto), desde qué choke point de base de datos, con qué payload y tenancy, y bajo qué garantías (atomicidad con la operación, degradación sin abortar la operación de negocio, unicidad de la activación por usuario). Cubre tanto la emisión en línea, vía trigger `AFTER INSERT` sobre `sales`/`purchases`/`expenses`, como el backfill histórico derivado de las operaciones ya existentes. Es la precondición de datos reales para activación, MAU, retención 30d, distribución de uso semanal y detección de UMV (capability `insights`).
...
Dos emisores para el mismo hecho es la duplicación que originó el defecto. El emisor legacy en `frontend/lib/supabase/services.ts` escribía la clave `expense_id` en lugar de `entity_id`, por lo que el índice de idempotencia no lo desduplicaría y produciría doble conteo.
...
El sistema SHALL derivar eventos históricos a partir de las filas existentes de `sales`, `purchases` y `expenses`, marcados con `source = 'backfill'` y sin duplicar eventos ya presentes.

Aprobado por el PO (OQ-2, 2026-08-12): sin este backfill, los usuarios ya existentes al momento del deploy de la emisión quedarían fuera de toda cohorte de `rpc_admin_retention_30d` de forma permanente, no solo durante la transición. Se despliega en una migración separada de la emisión (`20260919000001_analytics_events_backfill.sql`), de modo que la reparación del defecto no dependiera de esta decisión. El `created_at` de cada evento derivado es el `created_at` de la operación de origen. La deduplicación considera tanto el índice único por `entity_id` (cubre reemisiones del propio backfill y operaciones ya cubiertas por el trigger) como los eventos legacy preexistentes con claves `sale_id`, `purchase_id` o `expense_id`, que no comparten esa clave y por eso requieren su propio chequeo `NOT EXISTS`.
...
- **WHEN** el backfill alcanza una operación que ya tiene evento, emitido en línea, por el path legacy, o por una corrida anterior del propio backfill
...
### Requirement: Unicidad de la UMV por usuario
El sistema SHALL garantizar mediante un índice único parcial que exista como máximo un evento `umv_reached` por `user_id`, cerrando para esa métrica la misma clase de carrera bajo concurrencia que afecta a `first_operation`.

Aprobado por el PO junto con el backfill (OQ-3, 2026-08-12). El índice `ux_analytics_umv_reached_user` sigue el mismo patrón que `ux_analytics_first_operation_user`: la limpieza de duplicados históricos es idempotente y corre antes de crear el índice.

#### Scenario: Un segundo umv_reached para el mismo usuario es rechazado
- **WHEN** se intenta insertar un evento `umv_reached` para un usuario que ya tiene uno
- **THEN** la base de datos rechaza la inserción por violar el índice único parcial
