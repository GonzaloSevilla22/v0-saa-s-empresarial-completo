...
- **El límite de productos del plan queda FUERA de este change** (OQ-1, sin sign-off del PO — ver `design.md` §OQ-1). El diseño original evaluaba el límite **después** de escribir y **antes** de confirmar, contra `get_effective_plan` + `plan_limits` (las mismas fuentes canónicas), rechazando el lote entero con el conteo exacto si dejaría la cuenta por encima de su tope — pero rechaza importaciones de 4 cuentas reales medidas, así que exigía sign-off explícito del PO antes de escribirse. Sin esa respuesta, `rpc_import_products` no lo implementa: `P0430` queda reservado en `backend/core/errors.py` para cuando se active.
- **Vista previa validada por el servidor**: el paso 2 del diálogo deja de ser sólo validación de cliente. La misma RPC en modo simulación (`p_dry_run`) ejecuta el lote completo y lo deshace, devolviendo el veredicto real —incluidos los errores de fila y las categorías que se crearían— **antes** de escribir nada. (El veredicto de plan no viaja: ver el punto anterior.)
...
### New Capabilities
- `product-import`: la importación de productos por archivo como **operación de lote atómica servida por la API**: contrato de la fila normalizada, reparto de responsabilidades entre parseo de cliente y validación de servidor, todo-o-nada con reporte de errores por fila numerada, vista previa validada por el servidor, tope de filas, idempotencia por clave y dedupe por archivo, y superficie del diálogo de importación. (El enforcement del límite de plan al importar NO forma parte de este change — OQ-1 sin sign-off, ver `design.md` §OQ-1.)
...
- `plan-gating`: **queda sin tocar en este change.** El diseño original declaraba el enforcement del límite de productos extendido a la carga masiva, pero OQ-1 no tiene sign-off del PO (ver `design.md` §OQ-1) — el delta de `plan-gating` documenta la cláusula normativa para cuando se active, sin que ningún código de este change la implemente todavía.
...
**Riesgo de dominio**: gobernanza **MEDIA**. No escribe dinero: escribe catálogo, stock y categorías. El tramo delicado no es contable sino de **disponibilidad** — el todo-o-nada convierte un archivo con una fila mala en un archivo rechazado. (El riesgo del enforcement del límite de plan sobre cuentas que hoy importan no aplica a este change: OQ-1 quedó sin implementar, sin sign-off del PO.)
