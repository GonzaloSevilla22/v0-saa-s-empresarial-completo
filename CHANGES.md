...
**Change `asiento-venta-formulario`** ✅ **COMPLETADA 2026-08-20** (PR #431 apply — pendiente archive) — **cierra el agujero contable que quedaba abierto en la OQ-B de `pagos-cableados-restantes` (arriba)**: `rpc_create_sale_operation_v2` (la ruta viva del formulario de venta) no emitía ningún evento al outbox, de modo que **228 operaciones de venta del formulario ($13.333.157,19)** —el 66% de las ventas de la app— no tenían asiento contable. El lado compras resultó no tener agujero vivo: `rpc_create_purchase_operation` emite `PurchaseCreated` al 100% desde su alta (2026-06-29); las 40 restantes son backfill histórico, no un bug.
  ...
  - Specs sincronizadas: `journal-entry` (+2 requirements ADDED — `SaleOperationCreated`/`SaleOperationAdjusted`, +1 MODIFIED), `transactional-outbox` (+1 requirement ADDED, +2 MODIFIED), `operation-edit-context` (1 MODIFIED — aclara que el asiento NO es causa de bloqueo). Pendiente de archivar.
...
- ~~`asiento-venta-formulario`~~ — **implementada 2026-08-20** (ver entrada propia más abajo). Queda `delete-guard-ledgers` y la superficie del libro diario (OQ-5) como candidatos derivados todavía abiertos.
