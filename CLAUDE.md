...
11. **`tenancy-guard-caja-outbox`** 🔨 **PROPUESTO 2026-08-23** (propose PR **#459**, `openspec/changes/tenancy-guard-caja-outbox/`) — 🛑 **governance CRÍTICO: el apply NO arranca sin sign-off explícito del PO** (task 0.1 de `tasks.md`; el PO autorizó *proponer*, no *aplicar*). Cubre juntos **h1 + h2**, los dos hallazgos laterales de severidad **alta** y **reproducidos** que dejó `cuenta-corriente-party-guard`. **h1**: el POS acepta un `p_cash_session_id` de otro tenant y le escribe en la caja (`_c29_confirm_order_core` sólo chequea `IS NULL` y lo pasa crudo a `c28_register_cash_movement`, que es `SECURITY INVOKER` y no mira `account_id`; alcanzable por `rpc_quick_sale`/`rpc_confirm_sales_order`). El **formulario** de venta sí lo cierra con `cash_optin_requires_open_session`/`P0422` — el guard existía y no se replicó. Exposición medida: **los 65 movimientos de caja vivos de prod pasaron por ese único camino**, con 3 sesiones abiertas de 3 tenants. **h2**: `rpc_process_outbox_batch`/`rpc_mark_event_processed` devuelven y marcan eventos de todos los tenants a cualquier `authenticated`, agravado por `POST /outbox/process-pending` sin gating de admin. **Hallazgo nuevo del propose (h2 bis, verificado)**: hay **dos relays compitiendo por el mismo flag** — el dispatcher SQL corre 4 consumers (incl. JournalEntry y Notification) y el relay Python corre 2 y marca `processed_at` igual, así que todo evento que gane el Python **pierde para siempre su asiento contable**; el relay Python además **incumple el spec vigente** de `transactional-outbox`, el pg_cron no lo llama, y `v31-tenancy-pool-rls` ya lo tiene marcado como colisión del Paso 2. **El change**: guard de sucursal en `_c29_confirm_order_core` (`P0422`, predicado copiado del formulario) + backstop de tenant en `c28_register_cash_movement` (`P0401`, resolución por FKs, **sin cambiar la firma**) + el endpoint del outbox pasa a `get_service_conn` + `require_platform_admin` + `rpc_process_outbox_dispatch`, se retira `OutboxRelayService`, y recién entonces se revocan las dos RPCs + chequeo **(5)** permanente en el gate de ACLs (lista curada de funciones que leen/actualizan `public.events` — el (4) no las atrapa: excluye `rpc_*` por diseño) + auditoría read-only. Migración `20261012000001`. **La decisión difícil**: el `REVOKE` a secas rompería con el Paso 2 del pool, y filtrar por tenant sería incorrecto (el relay debe leer cross-account por spec) — la salida es el **camino de servicio**, que por contrato de `v31-tenancy-pool-rls` D5 nunca adopta `authenticated`, anclado con un test nuevo (OQ-6). **Sin superficie frontend** (declarado y verificado). Daño histórico preliminar **0/3**. 8 OQs con recomendación; **OQ-0 🛑** es el sign-off y **OQ-1 🛑** pregunta si h2 sale antes como hotfix al estilo #446/#454 (*recomendación: sí h2, no h1*). **Próximo: 🛑 sign-off del PO → `/opsx:apply tenancy-guard-caja-outbox`.**
