...
- **Pendiente**: 13.2-13.5 del `tasks.md` (esperar checks/mergear, verificación post-merge en prod, humo real con el PO, verificar legibilidad de los 7 históricos) — fuera de las reglas duras del apply ("no esperes CI, reportá el número y terminá"), quedan para el siguiente turno o el PO. Candidato registrado: **OQ-5** — configurar destinos bancarios por defecto en las 266 formas de pago (D4 lo dejó fuera de alcance explícitamente); un índice sobre `payment_method_id` en `payments_received`/`payments_made` quedó sin agregar por no medirse el plan (6/1 filas hoy no lo justifican).
- **Leer antes**: `openspec/changes/cobranzas-catalogo-pagos/design.md` (D1-D11 + las 5 OQs con su sign-off), `openspec/changes/archive/2026-09-02-cobranzas-reverso/` y `openspec/changes/archive/2026-09-02-caja-compras-cobranzas/` (el molde de las tres capas — RPC, backend, frontend — que este change repite por cuarta vez).
