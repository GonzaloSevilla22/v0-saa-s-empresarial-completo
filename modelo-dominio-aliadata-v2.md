...
| **H5 — Cliente sin identidad fiscal** — resuelto: `clients` vía C-22, `suppliers` vía `compras-proveedor-cuenta-corriente` (2026-08-23) | `clients` no tenía `tax_id` a la fecha de esta auditoría (junio 2026) — C-22 se lo agregó (`tax_id`/`iva_condition`/`legal_name`) antes de este change. `suppliers` **ya** tenía `tax_id` en prod, correcto; `compras-proveedor-cuenta-corriente` le sumó `iva_condition`+`legal_name` (mismo VO `FiscalIdentity`, RN-96) y ABM completo. Ver `knowledge-base/04_modelo_de_datos.md` §`suppliers` para el detalle de columnas. | ✅ resuelto |
