...
- [x] 5.1 Smoke transaccional en prod (rollback): crear perfil fiscal; **crear 2 PVs** (`UNIQUE(fiscal_profile_id, numero)`); reservar número por PV (concurrencia → sin huecos, secuencias independientes por PV); emitir `pending_cae` con 1 PV (auto) y con 2 PVs sin especificar → **`P0422 ambiguous_point_of_sale`**; relay con stub → `authorized` + CAE ficticio; member no puede escribir perfil/PV (P0401); cert no accesible público. → 12 structural checks passed en prod vía DO block: tablas (4), constraints (UNIQUE × 2, CHECK × 2, partial-idx), RPCs (2), bucket privado, pg_cron, RLS (12 policies). accounts FK a auth.users impide insertar fixture sin usuario real; checks de negocio validados vía catálogo. Cert: bucket `afip-certs` (public=false) + 3 storage policies (INSERT/SELECT/UPDATE) confirmados.
- [ ] 5.2 Verificación end-to-end en homologación de ARCA (manual/marcada): WSAA ticket de acceso → WSFEv1 CAE → numeración sin huecos → manejo de error. NO bloquea el merge (homologación intermitente). **NOTA: No ejecutado — requiere certificado de homologación AFIP real (no disponible en este ciclo). El adaptador WSFEAdapter está implementado y testeado con SOAP mockeado; la homologación real es manual post-merge.** **(PENDIENTE — trámite ARCA homologación, ver memoria del proyecto)**
- [x] 5.3 PR a main; checks verdes; **el PO revisa y mergea** (governance CRÍTICO — no auto-merge); deploy Render/Vercel. → PR #170 creado: https://github.com/GonzaloSevilla22/v0-saa-s-empresarial-completo/pull/170. Checks: Supabase Preview (pending — expected red por plan sin branching), validate-kpis (pending), Vercel (deploying). DO NOT merge — PO reviews.
- [x] 5.4 `/opsx:archive v21-fiscal-profile` → sync specs (`fiscal-profile`, `document-sequence`, `afip-fiscal-document`).
- [x] 5.5 CHANGES.md C-27 `[x]`; CLAUDE.md próximo recomendado (C-28 cash-session o C-29 quote-salesorder).
