...
- **`deudas-menores-agosto`** ✅ (2026-08-18, PR #413 apply/merge — **archivado** el mismo día) — cinco deudas menores de changes ya cerrados, sign-off PO 2026-08-18, ejecutadas en una sola pasada. **G1**: `sale_items_rpc_v2` pasa de opt-in a opt-out (ausencia de fila en `account_feature_flags` = v2) en `rpc_create_sale_operation`/`rpc_create_purchase_operation`, con UPSERT `enabled=true` por cuenta y rollback de una línea. **G2**: `clients.status` legacy pierde su superficie de lectura/edición en la UI (columna en DB intacta). **G3**: `/clientes` abre ordenado por última compra descendente por defecto (cierra OQ-4 de `clientes-frecuentes-historial`). **G4**: limpieza idempotente de `operation_created` huérfanos/duplicados en `analytics_events` (re-derivado, ninguna cuenta hardcodeada) + backfill de `account_id` en `sale_items`/`purchase_items`. **G5**: código muerto removido de `frontend/lib/adminAnalytics.ts` (cierra OQ-6 de `admin-kpi-refresh`).
  ...
  - **Deudas abiertas con dueño**: OQ-1 (`rpc_atomic_update_sale_operation`/`rpc_atomic_update_purchase_operation` no tocan `sale_items`/`purchase_items` — hueco mayor que el que este change cerró; **recomendado como próximo change, con prioridad**), OQ-2 (backfill de las 58 ventas históricas de agosto sin línea, junto con OQ-1), OQ-3 (RPCs `get_admin_*` sin consumidor siguen en la base).
