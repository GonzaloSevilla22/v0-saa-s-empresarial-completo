...
**Change `compras-proveedor-cuenta-corriente`** ✅ **COMPLETADA 2026-08-23** (apply PR #452 squash `2819772` — 9 commits incl. 3 tandas de review adversarial — + fix post-demo PR #455 `40704f0` + archive con specs sincronizadas: `supplier-directory` y `data-api-endpoints` nuevas, `supplier-account`/`party-account-charge`/`payment-method`/`operation-edit-context` modificadas). **Demo real del PO en prod ejecutada** (E2E con dinero real: proveedor → compra a crédito → pago → segunda compra → borrado con compensación `debit_note` y saldo $0 verificado en DB); los dos defectos que la demo destapó eran de UI y se corrigieron en #455 (cuentas corrientes sin invalidar en compras/ventas/POS + doble signo/dirección por tipo en el historial de proveedor) — cierra el agujero que quedaba explícito por escrito desde `pagos-cableados-restantes`: la cuenta corriente de proveedores existía entera desde C-30 (tablas, RPCs, backend de 3 capas, pantalla `/proveedores/[id]/cuenta`) y **nunca se usó** — 0 proveedores en prod, sin ABM, sin selector en el form de compra, sin cargo real.
  ...
  - **Coordinación con `cuenta-corriente-party-guard`** (propuesto 2026-08-22, artefactos en `openspec/changes/cuenta-corriente-party-guard/`, reservó `20261008000001`, **todavía sin apply**): sin conflicto de archivos; D6 de este change mantiene el chequeo de pertenencia del proveedor dentro de `rpc_create_purchase_operation` como defensa en profundidad, así que el camino nuevo no depende del orden de merge. Quien archive segundo de los dos rebasea sus deltas de `supplier-account`/`party-account-charge` sobre la spec ya sincronizada por el otro.
