-- =============================================================================
-- MIGRATION: 20260607000003_fix_idempotency_conflict_target.sql
-- BUG FIX:   ON CONFLICT target incorrecto en rpc_create_sale_operation
--            y rpc_create_purchase_operation
--
-- CAUSA RAÍZ:
--   20260531230737_idempotency_constraint_fix.sql eliminó el constraint
--   UNIQUE(user_id, idempotency_key) y lo reemplazó por
--   UNIQUE(user_id, operation_kind, idempotency_key).
--
--   Pero 20260607000001_rpc_branch_support.sql (C-07) reescribió ambas RPCs
--   usando el target viejo de 2 columnas → PostgreSQL no encuentra el constraint
--   → "there is no unique or exclusion constraint matching the ON CONFLICT
--   specification" al crear ventas o compras.
--
-- FIX:
--   Reemplazar ON CONFLICT (user_id, idempotency_key) por
--   ON CONFLICT (user_id, operation_kind, idempotency_key) en ambas RPCs.
--   El resto de las funciones queda idéntico.
--
...
-- ─────────────────────────────────────────────────────────────────────────────
-- rpc_create_sale_operation — fix ON CONFLICT target
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- rpc_create_purchase_operation — fix ON CONFLICT target
-- ─────────────────────────────────────────────────────────────────────────────
