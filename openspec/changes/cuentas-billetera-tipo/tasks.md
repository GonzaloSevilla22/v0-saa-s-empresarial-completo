...
- [ ] 9.1 Aplicar la migración y verificar en prod, con SELECTs, que las 4 cuentas activas y las 4 soft-deleted quedaron en `'wallet'` y que ninguna fila quedó fuera del dominio — **pendiente hasta el merge del PR 1** (escritura en prod solo vía la migración del pipeline, per regla del proyecto); se verifica POST-MERGE
...
- [ ] 9.5 Registrar en `CHANGES.md` el change y su resultado, y dejar anotadas OQ-1 (editar el tipo de una cuenta existente) y OQ-2 (fila con `currency = 'pesos'`) — se hace en la **Etapa 2 (archive)**, siguiendo la convención del roadmap (entradas de `CHANGES.md` se registran al archivar, con el resultado final ya conocido)
