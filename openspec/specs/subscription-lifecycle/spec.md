...
Ciclo de vida de las suscripciones recurrentes de MercadoPago (`preapproval`) que sostienen los planes pagos de una cuenta: un `preapproval_plan` de catálogo por tier, alta de la suscripción con referencia externa a la cuenta pagadora, unicidad de suscripción viva por cuenta garantizada por la base, sincronización del estado local con el que informa MercadoPago, extensión del período pagado en cada cobro mensual aprobado, dunning sin degradación inmediata ante un cobro fallido, cancelación con degradación diferida al fin del período pagado (tanto voluntaria como por impago), aviso de vencimiento próximo, y aislamiento de los datos de suscripción por cuenta vía RLS. Implementado en `mp-real-subscriptions` (2026-08-01/02).
