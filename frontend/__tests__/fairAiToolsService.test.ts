 ...
 * replicado acá. `processTimeSeries` alimentaba el gráfico "Evolución
 * temporal" de /admin/feria-ia con `{ name: "Ene".."Dic", value }` —
...
import { fairAiToolsService } from "@/lib/services/fairAiToolsService"

describe("fairAiToolsService.processTimeSeries", () => {
  it("n=0: sin filas produce una serie vacía (no un bucket basura)", () => {
    expect(fairAiToolsService.processTimeSeries([])).toEqual([])
  })

  it("n=1: una sola fila produce un único punto con activations=1 y period parseable como fecha real", () => {
    const result = fairAiToolsService.processTimeSeries([{ created_at: "2026-03-05T12:00:00.000Z" }])
  ...
  it("REGRESSION: el shape es { period, activations } directo — sin name/value/umv_achieved inventados", () => {
    const result = fairAiToolsService.processTimeSeries([{ created_at: "2026-03-01T00:00:00.000Z" }])
  ...
  it("REGRESSION: agrega por AÑO+mes — marzo 2026 y marzo 2027 no se mezclan en el mismo bucket", () => {
    const result = fairAiToolsService.processTimeSeries([
  ...
  it("cada period cae en el primer día del mes (UTC), no en la fecha exacta de la fila", () => {
    const result = fairAiToolsService.processTimeSeries([{ created_at: "2026-06-27T18:45:00.000Z" }])
