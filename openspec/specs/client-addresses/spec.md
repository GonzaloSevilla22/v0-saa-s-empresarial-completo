...
Direcciones operativas múltiples por cliente (Modelo V3 §7.3), con `alias`, invariante de exactamente una dirección primaria viva por cliente, soft-delete alineado a la política de maestros (V3 §4), y switch atómico de la primaria vía RPC. Distinta de la dirección **fiscal**, que vive en la identidad fiscal del cliente (`client-fiscal-identity`) y es inmutable por snapshot en los documentos — ese capability no cambia.
