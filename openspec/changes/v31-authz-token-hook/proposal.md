...
- **El hook pasa a emitir tres claims bajo `app_metadata`, con espacios de nombres separados**:
  ...
  - `plan` — **claim nuevo**, plan **efectivo** de la cuenta activa (`accounts.billing_plan`, con trial vigente teniendo precedencia, según la capability `plan-gating`). En prod hay 5 cuentas con trial activo: omitir el trial las degradaría.
...
- El límite de productos por plan pasa a ser real. 8 cuentas `gratis` bajan de 999.999 a 100 productos; **una de ellas ya está en 123 y quedaría bloqueada para crear productos nuevos**. Esto no está cubierto por el sign-off del 2026-07-30 → **OQ-1 para el PO**, con tarea-gate previa a la activación.
...
- **Base de datos**: una migración idempotente que redefine `public.custom_access_token_hook(jsonb)` (`CREATE OR REPLACE`, misma firma — sin riesgo de overload duplicado) + `GRANT SELECT` y policies de lectura para `supabase_auth_admin` sobre `account_members` y `accounts`. Sin DDL de tablas, sin backfill, sin borrado de datos.
...
- **Governance**: **CRÍTICO** — toca la emisión de tokens de 34 usuarios reales. Análisis, migración y código se preparan y testean; **la activación en producción requiere acción y sign-off explícitos del PO**, y la resolución previa de OQ-1.
- **Cluster**: prerequisito duro de `v3-rbac-multirole`. Corre en paralelo lógico con `v31-tenancy-pool-rls`; ambos deben cerrar antes del pivot. Depende de `v31-fix-auth-shape-500` ✅ (el contrato `AuthContext` que este change extiende).
