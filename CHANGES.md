...
- **Estado**: 🔨 **PR1-4 mergeados 2026-08-01** (esquema + backend + frontend + correos completos, 38/76 tasks) · **Governance**: CRÍTICO (dinero real) — sign-off PO 2026-07-31 sobre el rumbo + amendment/pivot 2026-08-01; **activación en producción sigue apagada** (palanca `billing_subscriptions_enabled=False`)
...
- **Pendiente**: tareas §9 (activación, todas `[MANUAL PO]`) — ver `openspec/changes/mp-real-subscriptions/tasks.md`. **H-19 / `v31-mp-webhook-atomic`** (atomicidad transaccional del webhook) sigue abierto — este change no lo cubre, mismo alcance que `v31-mp-upgrade-webhook-fix`.
- **Leer antes**: `openspec/changes/mp-real-subscriptions/design.md` (Amendment + D2bis, D6, D9-D11), `openspec/changes/mp-real-subscriptions/tasks.md`, `v31-mp-upgrade-webhook-fix` (prerequisito duro).
